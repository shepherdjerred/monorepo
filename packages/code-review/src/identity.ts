 ...
 * appends `[bot]`. Strip the suffix before comparing, then match by
 * case-insensitive substring so both the exact slug and any variant resolve.
 */
...
/** True when `login` belongs to `provider`. */
export function isProviderAuthor(
  ...
  return provider.authorLogins.some((token) =>
