...
- **Claude Code CLI's `--json-schema` does not accept the strict/nullable dialect** — it silently exits 0 with no `structured_output` field at all, rather than erroring. `homelab-audit-daily` ran cleanly for 100+ days on the old plain schema; 07-30 is the first run where this exact error appears cleanly (07-29's run was separately masked by a Claude weekly-quota 429; 07-17→07-28 had a distinct, still-unexplained non-zero-exit failure — see the two new todos filed below). No other code touching this path changed in that window.
...
- File two new `packages/docs/todos/` entries for issues discovered but out of scope here (per "fix X means X" — don't fold unrelated bugs into this PR):
  - **Codex `ci-io-post-merge-impact` bwrap sandbox failure**: every run's shell commands fail with `bwrap: No permissions to create a new namespace` (codex's `--sandbox read-only` needs a Linux namespace the worker pod can't create). A same-day commit `cda4e819e` already switched this to `--sandbox danger-full-access`, unverified in production as of this writing — file as a verification todo to confirm the next scheduled run.
