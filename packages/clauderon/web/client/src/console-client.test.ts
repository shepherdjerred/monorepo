...
function getWs(client: ConsoleClient): MockWebSocket {
...
describe("ConsoleClient - constructor", () => {
...
describe("ConsoleClient - connect", () => {
...
describe("ConsoleClient - disconnect", () => {
...
describe("ConsoleClient - write", () => {
  ...
  test("sends base64-encoded input message", async () => {
    ...
    const parsed: any = JSON.parse(messages[0]!);
    expect(parsed.type).toBe("input");
    expect(atob(parsed.data)).toBe("hello");
  });
});

describe("ConsoleClient - resize", () => {
  test("throws when not connected", () => {
    const client = new ConsoleClient();
    expect(() => {
      client.resize(24, 80);
...
describe("ConsoleClient - UTF-8 streaming edge cases", () => {
  ...
  test("decoder state resets on disconnect", async () => {
    ...
    const clientAny: any = client;
    expect(clientAny.decoder).not.toBeNull();

    client.disconnect();

    expect(clientAny.decoder).toBeNull();
  });
});

describe("ConsoleClient - listener and connection state", () => {
...
describe("ConsoleClient - base64 validation and error handling", () => {
...
describe("ConsoleClient - DecodeError with rich context", () => {
  test("validation stage error includes context", async () => {
    ...
    expect(onError).toHaveBeenCalledTimes(1);
    const error: any = onError.mock.calls[0]![0];
