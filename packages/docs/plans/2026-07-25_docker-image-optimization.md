...
- All 9 app Dockerfiles converted to the 4-stage slim pattern; every image
  built + in-image smoke-solved locally via the CI path (`--set
<t>.target=smoke`). Sizes: 6825 → 4259 MiB compressed (−38%); churn checks
  verified.
- Selector precision: content-aware root-manifest attribution, version-exact
  per-dep patches/ attribution, narrowed `.buildkite/` globals; 34 tests.
- ci-base: baked bun cache removed (−988 MiB); `buildkite-bun-cache` PVC +
  agent-stack pod-spec-patch mount/env in `buildkite.ts`; concurrency spike
  passed; cdk8s typecheck/build/tests green.
- Rebased onto the no-dind cutover (#1663) + scout ranked designs (#924) +
  scout tag-mint fix (#1666); reconciled smoke/image stages (retargeted to
  `runtime` — bare-`base` merge would have pushed empty images), kept both new
  PVCs, staged the smoke harness from context.
- `bun run verify -- --affected` green (60/60 tasks); **PR #1668** (draft)
  opened via git-spice with full body + size table.

### Remaining

- Promote PR #1668 from draft after Buildkite goes green; merge is the
  fleet-redeploy event (watch Argo health per service).
- Post-merge watch: next unrelated main build must log `content unchanged … no
version bump` for every converted image.
- Decide on the two stale patches (`twisted@1.73.0`, `satori@0.18.3` — apply
  to nothing in the current lockfile): re-pin or delete.

### Caveats

- After-sizes are local arm64 `docker save | gzip`; amd64 CI sizes will differ
  slightly. First main build rebuilds everything (global inputs changed) and
  bumps every versions.ts pin — one-time by design.
- The shared bun-cache PVC mounts only into `container-0`; multi-container
  steps' other containers fall back to per-pod cache paths (graceful).
- The bun-cache PVC + pod-spec-patch deploy via ArgoCD on merge; until synced,
  CI installs run network-cold (ci-base no longer bakes a cache).
