...
- **Agents are ephemeral (pod-per-job), never persistent.** Every recurring CI pathology here came from persistent state under concurrency (Dagger 2 TiB cache disease, bun EEXIST races on shared installs — re-reproduced by moon's auto-install during the PoC). Warmth comes from three safe cache layers instead: turbo remote cache (object storage), a node-local hostPath `BUN_INSTALL_CACHE_DIR` (content-addressed downloads only — `globalStore` stays off per oven-sh/bun#12917), and registry-backed BuildKit layers. Optional later: node-local git-mirror volume if clone latency annoys. Ephemeral pods also keep Kueue/max-in-flight governance meaningful.
