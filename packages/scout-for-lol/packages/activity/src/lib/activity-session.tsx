...
function assertContractHash(auth: CustomAuthResponse): void {
  const { contractHash } = activityEnvironment();
  if (contractHash !== undefined && auth.contractHash !== contractHash) {
    throw new Error(
...
async function startActivitySession(): Promise<ActivitySession> {
  ...
  const sdk = createDiscordSdkAdapter(clientId);
  await sdk.ready();
