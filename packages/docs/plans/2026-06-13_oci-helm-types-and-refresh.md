...
- **Part B — weekly refresh** (deterministic Temporal workflow, data-dragon pattern): new `activities/helm-types-refresh.ts` (clone → build eslint-config+helm-types → `generate-helm-types` → repo-prettier → `git add -- generated/helm` → `openSeasonRefreshPr`), thin `workflows/helm-types-refresh.ts`, registered in `activities/index.ts` + `workflows/index.ts`, `helm-types-weekly-refresh` schedule (`0 6 * * 1` PT). `withHelm` added to the temporal-worker image (`.dagger/src/image.ts`), copied from `HELM_IMAGE`.
...
- **Prettier split (load-bearing):** the generator's bundled prettier wraps differently than the repo's pinned prettier, and `generated/helm/` is NOT prettier-ignored — so the repo prettier is the authority. The Part B activity runs `bunx prettier --write generated/helm/` after generating; without it, every weekly run would open a formatting-only PR. (The generator's own internal prettier step is now redundant/misleading — a future cleanup could drop it.)
