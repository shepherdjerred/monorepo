...
#[derive(Clone, Debug)]
pub struct AppState {
    /// Core session manager for CRUD operations.
    pub session_manager: Arc<SessionManager>,
    /// Broadcaster for real-time WebSocket events.
    pub event_broadcaster: EventBroadcaster,
    /// Optional authentication state (None when auth is disabled).
    pub auth_state: Option<AuthState>,
    /// Console client tracking for terminal multiplexing.
...
async fn upload_file(
    ...
    #[expect(clippy::cast_possible_truncation, reason = "image upload files are never >4GB")]
