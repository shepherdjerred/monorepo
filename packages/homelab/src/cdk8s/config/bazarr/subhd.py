...
LAST_REQUEST = 0.0
AI_LABEL = re.compile(r"(?<![a-z])AI(?![a-z])|机器翻译|機器翻譯|机翻|機翻|自动翻译|自動翻譯|ChatGPT|DeepL|Whisper", re.I)
AI_CREDIT = re.compile(r"机器翻译|機器翻譯|机翻|機翻|(?:\bAI\b|ChatGPT|DeepL|Whisper).{0,24}(?:翻译|翻譯|校对|校對|润色|潤色|translat|transcrib|proofread)|(?:翻译|翻譯|校对|校對|润色|潤色|translat|transcrib|proofread).{0,24}(?:\bAI\b|ChatGPT|DeepL|Whisper)", re.I)
...
def is_season_pack(name, season):
    return not episode_numbers(name) and bool(re.search(r"(?i)s%02d(?!\d)" % season, name))


def pick_archive_member(names, season, episode, release="", explicit_simplified=False):
...
def validate_cdn_url(value):
    ...
    try:
        ...
        valid = (parsed.scheme == "https" and parsed.hostname == "dl.subhd.me"
...
def extract_subtitle(content, season, episode, release, explicit_simplified):
...
def parse_candidates(html):
    ...
    for block in soup.select("div.bg-white"):
        ...
        if not link or not version_node:
            continue
        sid = link.get("href", "").removeprefix("/a/")
        ...
        if not any(label in text for label in ("官方字幕", "原创翻译", "其他来源")):
            continue
        candidates.append({"sid": sid, "version": version_node.get_text(" ", strip=True),
                           "provenance": provenance, "bilingual": "双语" in labels,
...
class SubhdSubtitle(Subtitle):
    ...
    def __init__(self, candidate, release_info, video, language=SIMPLIFIED):
    ...
    @property
    def id(self):
        return "%s:%02d:%02d" % (self.sid, self.season, self.episode)
...
class SubhdProvider(Provider):
    ...
    def _instance(self):
        profiles = self._api("GET", "/profiles")
        ...
        if profile is None:
            profile = self._api("POST", "/profiles", {"name": self.profile})
        if not isinstance(profile, dict) or not isinstance(profile.get("id"), str):
            raise APIThrottled("subhd: invalid profile identity")
        instances = self._api("GET", "/instances")
        ...
        if instance is None:
            instance = self._api("POST", "/profiles/" + quote(profile["id"], safe="") + "/start",
        ...
        for _ in range(15):
            ...
            if not isinstance(instances, list):
                raise APIThrottled("subhd: invalid instance response")
            state = next((i.get("status") for i in instances if isinstance(i, dict) and i.get("id") == instance_id), None)
            if state == "running":
                return instance_id
            if state != "starting":
    ...
    def list_subtitles(self, video, languages):
        if not self.languages.intersection(languages) or not isinstance(video, Episode):
            return []
        candidates = {}
        for keyword in ("%s S%02dE%02d" % (video.series, video.season, video.episode),
                        "%s S%02d" % (video.series, video.season), video.series):
            for candidate in parse_candidates(self._get("/search/" + quote(keyword, safe="")).text):
        ...
        for candidate in ranked[:8]:
            html = self._get("/a/" + candidate["sid"]).text
            soup = ParserBeautifulSoup(html, ["lxml", "html.parser"])
            ...
            if detail and AI_LABEL.search(detail.get_text(" ", strip=True)):
                continue
            releases = [line.strip() for line in soup.get_text("\n", strip=True).splitlines()
            ...
            for name in releases:
                ...
                if {"series", "season"}.issubset(matches) and (not numbers or numbers == {(video.season, video.episode)}):
        ...
        return sorted(subtitles, key=lambda s: ("release_group" in s.get_matches(video), s.provenance, s.bilingual), reverse=True)

    def _wait_page(self, tab, path, prepare=False):
        for _ in range(12):
            state = self._api("POST", "/tabs/" + tab + "/evaluate", {"expression":
                "({ready:document.readyState,challenge:!!document.querySelector('#challenge-running,iframe[src*=challenges]')||/Just a moment|下载页面已失效/.test(document.title),path:location.pathname,prepare:!!document.querySelector('.subtitle-prepare-download')&&!!window.jQuery})"})
            result = state.get("result") if isinstance(state, dict) else None
            if isinstance(result, dict) and result.get("ready") == "complete" and result.get("path") == path and result.get("challenge") is False:
                if not prepare or result.get("prepare") is True:
                    return
            time.sleep(2)
        raise APIThrottled("subhd: download page unavailable or browser challenge requires human intervention")

    def _download_url(self, subtitle):
        instance = self._instance()
        opened = self._api("POST", "/instances/" + quote(instance, safe="") + "/tabs/open",
        ...
        try:
            ...
            prepare_expression = "(async()=>{const r=await fetch('/api/sub/prepare-download',{method:'POST',headers:{'Content-Type':'application/json','X-Requested-With':'XMLHttpRequest'},body:JSON.stringify({sid:" + json.dumps(subtitle.sid) + "})});if(!r.ok)return {prepared:false};const p=await r.json();if(p.success!==true||typeof p.url!=='string')return {prepared:false};const target=new URL(p.url,location.origin);if(target.origin!==location.origin||target.pathname!==" + json.dumps("/down/" + subtitle.sid) + ")return {prepared:false};setTimeout(()=>location.assign(target.href),100);return {prepared:true}})()"
            prepared = self._api("POST", "/tabs/" + tab + "/evaluate", {"expression": prepare_expression, "awaitPromise": True})
            ...
            self._pace()
            expression = "(async()=>{const r=await fetch('/api/sub/down',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({sid:" + json.dumps(subtitle.sid) + "})});if(!r.ok)return {success:false};return await r.json()})()"
            payload = self._api("POST", "/tabs/" + tab + "/evaluate", {"expression": expression, "awaitPromise": True})
            ...
            try:
                ...
                for _ in range(3):
                    with self.session.get(url, timeout=30, stream=True, allow_redirects=False) as response:
                        ...
                        subtitle.content = extract_subtitle(bytes(data), subtitle.season, subtitle.episode,
                                                            subtitle.video_release, subtitle.only_simplified)
                        return
