import type { VoiceAudioInput } from "@shepherdjerred/voice-assistant/types.ts";
import { DiscordOpusDecoder } from "./codecs.ts";
import type { KeywordDetectionEvidence } from "@shepherdjerred/voice-assistant/local-models.ts";
import {
  SpeakerRegistry,
  type SpeakerState,
} from "@shepherdjerred/voice-assistant/speaker-registry.ts";
import {
  VOICE_DTX_GAP_MS,
  VOICE_DTX_TICK_MS,
  VOICE_FRAGMENT_TAIL_MARGIN_MS,
  VOICE_VERIFICATION_DELAY_MS,
} from "@shepherdjerred/voice-assistant/constants.ts";
import { NOOP_VOICE_ATTEMPT_OBSERVER } from "@shepherdjerred/voice-assistant/realtime/attempt.ts";
import type {
  CompletedVoiceTurn,
  PendingVoiceTurn,
