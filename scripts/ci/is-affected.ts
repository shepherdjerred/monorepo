 ...
 *
 * Usage: bun scripts/ci/is-affected.ts <package-name>
 */
...
if (pkg === undefined || pkg === "") {
  console.error("Usage: bun scripts/ci/is-affected.ts <package-name>");
  process.exit(2);
}

// Spawned directly (not via lib/run.ts): stdout must carry ONLY the verdict —
// callers do `verdict=$(bun scripts/ci/is-affected.ts <pkg>)` — and the lib
