...
Three front-end surfaces (a marketing Astro site, a Vite React SPA, and a
Starlight documentation site) are built into one bucket per stage and served by
a shared S3-proxy Caddy that reverse-proxies `/api` + `/trpc` to the Scout backend.
...
| Documentation  | `docs-site/` | Astro + Starlight                | `/docs/`   | Same-release user documentation                          |
...
A single bucket must contain all three surfaces, because the CI deploy runs `aws s3 sync --delete` and would otherwise wipe whichever half is missing. `packages/scout-for-lol/scripts/build-bucket.ts`:

1. Builds the Astro marketing site → `packages/frontend/dist/`
2. Builds the Vite SPA → `packages/app/dist/`
3. Builds Starlight → `packages/docs-site/dist/`
4. Copies the SPA and docs into `packages/frontend/dist/app/` and `packages/frontend/dist/docs/`
5. Fail-fast asserts `dist/index.html`, `dist/app/index.html`, and `dist/docs/index.html` exist before any sync runs

Final layout in `packages/frontend/dist/`: marketing at `/`, SPA at `/app/`, and
Starlight documentation at `/docs/`. That `dist` is the `distDir` the CI deploy syncs.
