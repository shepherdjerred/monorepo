...
- **`stocks-sjer-red`** was the lone deploy target with no `aws_s3_bucket` in `buckets.tf` — wired into CI/Caddy/app (commit `6d0aa524b`) but never tofu-declared; SeaweedFS auto-created it on first deploy sync. Now declared + given a lifecycle rule, and adopted into state via a declarative `import {}` block (no manual `tofu import` — the post-merge `tofu-apply-all` imports it automatically; CI applies the `seaweedfs` stack from main, so config + import must land together, which they do). A manual pre-merge import would have been unsafe: it leaves state ahead of main's config, and the next homelab apply would plan to **destroy** the bucket. The import block is a no-op once imported and can be removed later. Aside: SeaweedFS's auto-create-on-PutObject means the other `aws_s3_bucket` resources are partly belt-and-suspenders — left as-is.
