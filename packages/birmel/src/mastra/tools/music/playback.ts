...
export const musicPlaybackTool = createTool({
  id: "music-playback",
  description: "Control music playback: play, pause, resume, skip, stop, seek, set volume, set loop mode, or get now playing",
  inputSchema: z.object({
    guildId: z.string().describe("The ID of the guild"),
    action: z.enum(["play", "pause", "resume", "skip", "stop", "seek", "set-volume", "set-loop", "now-playing"]).describe("The action to perform"),
    channelId: z.string().optional().describe("Text channel ID for responses (for play)"),
    voiceChannelId: z.string().optional().describe("Voice channel ID (for play)"),
    query: z.string().optional().describe("URL or search query (for play)"),
    seconds: z.number().min(0).optional().describe("Position in seconds (for seek)"),
    volume: z.number().min(0).max(100).optional().describe("Volume level 0-100 (for set-volume)"),
    loopMode: z.enum(["off", "track", "queue", "autoplay"]).optional().describe("Loop mode (for set-loop)"),
  }),
      ...
      switch (ctx.action) {
        case "play": {
          if (!ctx.channelId || !ctx.voiceChannelId || !ctx.query) {
            return { success: false, message: "channelId, voiceChannelId, and query are required for play" };
          }
          ...
          logger.info("Music playback started", { guildId: ctx.guildId, track: track.title });
          return {
            success: true,
        ...
        case "now-playing": {
          ...
          return {
            ...
            data: { title: track.title, duration: track.duration, url: track.url, progress: progress ?? "" },
          };
        }
      }
    } catch (error) {
