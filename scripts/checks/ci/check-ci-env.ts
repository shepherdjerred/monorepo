 ...
 *
 * Usage: bun scripts/checks/ci/check-ci-env.ts
 *
 * Exit codes:
...
const STEP_REQUIREMENT_EXCEPTIONS: readonly {
  ...
  },
  {
    ...
    reason:
      "Same shape via scripts/lib/scout/scout-site-storage.ts's requireCreds(dryRun): " +
  ...
  {
    step: "pr-dryrun",
    script: "packages/homelab/scripts/argocd/argocd.ts",
    names: ["ARGOCD_TOKEN"],
    reason:
  ...
  },
  {
    ...
    reason:
      ...
      "inference, GitHub App authentication, and the OpenAI credential " +
  ...
  for (const site of required.unresolved) {
    ...
    errors.push(
      `step "${step.key}" runs ${entry}, whose requireEnv at ${site} is not a string literal. ` +
        `Add an exception to DYNAMIC_CALL_EXCEPTIONS in scripts/checks/ci/check-ci-env.ts declaring the names it may require.`,
    );
  }
