<template>
  <div>
    <section
      ...
      :class="{ 'is-medium': !assignment.attachment }"
    >
      <div class="hero-body">
        <div class="container">
          <h1 class="title">
          ...
          <h2 class="subtitle">
            <template v-if="assignment.possiblePoints">
              <template v-if="assignment.graded">
                <p>
                  Graded, you earned {{ assignment.earnedPoints }} out of
              ...
              <template v-else>
                <p>
                  Not yet graded, worth {{ assignment.possiblePoints }} points
                </p>
              </template>
            </template>
          </h2>
            ...
            <div class="tabs is-centered is-boxed">
              <ul>
                <li class="nav-link is-active">
                  <a> <i class="fa fa-fw fa-file"></i> Attachment </a>
                </li>
                <li class="nav-link">
                  <router-link
                    :to="{
                      name: 'Course Details',
                      params: { id: assignment.course.id },
                    }"
                  >
                    <i class="fa fa-fw fa-book"></i> Back to
                ...
                <template v-if="assignment.type !== 'NOTES'">
                  <li>
                    <a
                      class="nav-link"
                      :href="
                        'https://cs.harding.edu/easel/cgi-bin/view?id=' +
                        assignment.id +
                        '&action=submit'
                      "
                      target="_blank"
          ...
          <div class="column is-full">
            <template v-if="assignment.attachment">
              <div>
                ...
                <a :href="assignment.attachment" target="_blank" class="button"
                  ><i class="fa fa-fw fa-external-link"></i> Open attachment in
                  new tab</a
                >
              </div>
            </template>
...
export default {
  ...
  methods: {
    dueDate: function (assignment) {
      return (
        Date.getMonthName(assignment.date.getMonth()) +
        " " +
