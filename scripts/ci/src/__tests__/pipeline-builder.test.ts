...
import { ALL_PACKAGES, SKIP_PACKAGES } from "../catalog.ts";
import {
  emptyAffected,
  fullBuild,
  commandSteps,
  ...
  collectKeys,
  collectKeysAndDeps,
  withBuildkitePullRequest,
  withDryRun,
  withBuildkiteBranchContext,
  homelabStepKeys,
  driftStep,
  versionBumpAffected,
  releasePleaseAffected,
...
describe("buildPipeline", () => {
  ...
  describe("bun.lock drift gate", () => {
    it("emits the gate with seeds from directlyChanged, not the closure", () => {
      ...
      // would silently miss the dpp/nested-workspace case (PR #1213).
      // Verifies the regression Greptile flagged against this PR.
      const affected = emptyAffected();
      affected.directlyChanged.add("llm-observability");
      affected.packages.add("llm-observability");
      // The closure as computed by change-detection.ts does NOT contain dpp
      // (its `file:` edge lives in a nested manifest, invisible to the
      // top-level-only `readWorkspaceDeps`).

      const step = driftStep(affected);
      expect(step).toBeDefined();
      expect(step!.command).toContain("--seeds llm-observability");
      // Must use --seeds, not --packages: the script expands the closure
      // itself using a nested-workspace-aware graph.
      expect(step!.command).not.toContain("--packages");
    });

    it("sorts seeds for stable commands", () => {
      const affected = emptyAffected();
      affected.directlyChanged.add("scout-for-lol");
      affected.directlyChanged.add("birmel");
      affected.directlyChanged.add("llm-observability");
      affected.packages.add("scout-for-lol");
      affected.packages.add("birmel");
      affected.packages.add("llm-observability");

      const step = driftStep(affected);
      expect(step).toBeDefined();
      expect(step!.command).toContain(
        "--seeds birmel,llm-observability,scout-for-lol",
      );
    });

    it("omits the gate when no packages directly changed", () => {
      // Version-commit-back / Renovate-noop-with-helm-types: `packages` may
      // contain "homelab" to drive other gates, but `directlyChanged` is
      // empty because no `packages/<X>/package.json|bun.lock` was touched.
      // The drift gate has nothing to check and must not emit.
      const affected = emptyAffected();
      affected.packages.add("homelab");
      affected.helmTypesInputsChanged = true;
      // directlyChanged stays empty

      expect(driftStep(affected)).toBeUndefined();
    });

    it("omits the gate on buildAll (per-package frozen installs cover it)", () => {
      const affected = fullBuild();
      // Even if we somehow seed directlyChanged on a buildAll path, the
      // per-package Dagger jobs each run `bun install --frozen-lockfile`,
      // so the gate adds no signal and must be skipped.
      affected.directlyChanged.add("llm-observability");

      expect(driftStep(affected)).toBeUndefined();
    });

    it("emits the gate for a normal scoped PR with seeds", () => {
      const affected = emptyAffected();
      affected.directlyChanged.add("birmel");
      affected.packages.add("birmel");
      ...
      const step = driftStep(affected);
      expect(step).toBeDefined();
      expect(step!.command).toContain("--seeds birmel");
    });
  });
...
// Split from the `buildPipeline` describe above so the container callback
// stays under max-lines-per-function.
describe("buildPipeline (structure & release gating)", () => {
