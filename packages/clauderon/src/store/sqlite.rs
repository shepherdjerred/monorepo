...
impl SqliteStore {
    ...
    /// Migration v18: Add PR merge fields (review status, merge methods, delete branch setting)
    async fn migrate_to_v18(pool: &SqlitePool) -> anyhow::Result<()> {
        tracing::info!("Applying migration v18: Add PR merge fields");
    ...
    fn try_from(row: SessionRow) -> Result<Self, Self::Error> {
        ...
        Ok(Self {
            ...
            #[expect(
                ...
                reason = "port numbers are always within u16 range"
            )]
            ...
            #[expect(
                clippy::cast_possible_truncation,
                clippy::cast_sign_loss,
                reason = "reconcile attempts bounded by application logic"
            )]
...
struct EventRow {
    ...
    #[expect(
        dead_code,
        reason = "mapped from DB column but payload is used for deserialization"
    )]
    event_type: String,
    payload: String,
    timestamp: String,
