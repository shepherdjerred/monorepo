...
When measuring observed impact, load `scoutql`, split matches on the exact patch boundary, and report games in both periods. Compare the same population and queue where possible. State separately: what Riot changed, what changed in Scout's sample, and plausible interpretations. Use cautious wording for small samples and do not call correlation causation.
