...
true false false`);
  ...
  describe("exceptions", () => {
    it("should yield exception messages", () => {
      ...
      return render(text, context).then((result) => {
        should(result)
          .be.a.String()
          .equal("TypeError: Cannot read property 'y' of undefined");
      });
    });
  ...
  describe("undefined", () => {
    ...
    it("should handle properties of undefined values", () => {
      let text =
        "-[[undefined.foo]]-[[null.foo]]-[[goo.foo]]-[[fee.fie.foe.fum]]-";
  ...
  describe("helpers", () => {
    ...
    it("should allow blocs to refer to bloc dictionary using this", () => {
      let text =
        "[[+ 2 * this.pi * this.r]][[pi: 3.14159]][[r: 10]][[- 2 * this.pi * this.r]]";
      return render(text).then((result) => {
        should(result)
          .be.a.String()
          .equal(String(3.14159 * 20));
      });
    });
  ...
  describe("nested templates", () => {
    ...
    it("should allow templates to access bloc properties", () => {
      let text =
        '[[+this.contents]][[name: "Joe"]]Hello, [[bloc.name]][[-this.contents]]';
      return render(text).then((result) => {
...
[[-if]]`) as Helper;
    ...
    it("should use require to read template files", () => {
      ...
      return render(text, context).then((result) => {
        should(result)
          .be.a.String()
          .equal("Unexpected character in bloc at testrc/error.blx:1:14");
      });
    });
  });
});
