...
try {
  ...
  process.stdout.write(
    [
      ...
      `  packet_ready p95 = video ${dash(summary.stream.packet_ready_delay_video_p95, 1)} / audio ${dash(summary.stream.packet_ready_delay_audio_p95, 1)} ms`,
      `  send_complete p95= video ${dash(summary.stream.send_complete_delay_video_p95, 1)} / audio ${dash(summary.stream.send_complete_delay_audio_p95, 1)} ms`,
      `  A/V content       = mean ${dash(summary.stream.av_content_offset_ms.mean, 1)} / p95 abs ${dash(summary.stream.av_content_skew_abs_ms_p95, 1)} ms (+ audio lags)`,
      `  correlation errs  = ${String(summary.stream.latency_correlation_failures_delta)}`,
      "input:",
      ...
      `  input→packet p95 = ${dash(summary.input.input_to_packet_ready_ms_p95, 1)} ms`,
      `  input→send p95   = ${dash(summary.input.input_to_send_complete_ms_p95, 1)} ms`,
      "",
    ].join("\n"),
  );
