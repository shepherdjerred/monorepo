...
| E   | ci-base digest pin: push `:v<build#>`; refresh step opens auto-merge bump PR rewriting the pinned tag in pod anchors; `imagePullPolicy: IfNotPresent`                                                                                                                                                                 | `.buildkite/scripts/build-ci-image.sh`, `.buildkite/pipeline.yml`, small bump script                                                              |
...
- [ ] Component D: PR micro-lane merge + main npm/cooklang merge +
      validate-pipeline invariant updates
- [ ] Component E: ci-base digest pin + refresh bump PR + IfNotPresent
- [ ] Submit the PR (git-spice), drive `buildkite/monorepo/pr` green
- [ ] Post-merge: watch first main build (buildkitd bake, tmpfs canaries,
