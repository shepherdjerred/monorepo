...
export function spawnWatcher(
  ...
  const watcherEntrypoint =
    options.controlSocket === undefined ? "watch.ts" : "live-watch.ts";
  // The dashboard entrypoints are siblings of this module; the child keeps the
  // package's `src/` as its working directory, as it did before `src/` was
  // split into layers.
