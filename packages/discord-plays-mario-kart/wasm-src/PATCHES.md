...
| `0002-makefile-exported-functions.patch` | `code/Makefile` | Adds the complete headless host export list to `EXPORTED_FUNCTIONS`, including `_malloc` for ROM injection and the RDRAM accessors, and keeps the build outputs in `code/` (drops upstream's `mv … ../dist/`) so the build scripts collect them. Leaves the upstream optimization flags untouched (`ASSERTIONS=0`, no profiling) for a lean prod build. |
| `0003-rdram-export.patch` | `code/mymain.cpp` | Adds `neilGetRdram()` / `neilGetRdramSize()` — base + size of the core's `g_rdram` array (referenced directly: this build links `libretronew.c`, which has no `retro_get_memory_data`), so the host can read game state (race placements, course id, timers) for leaderboards. See the byte-order contract below. |
| `0004-emscripten-6-pointer-types.patch` | `code/Makefile`, `code/src/libretro/neil.h`, `code/src/libretro/libretronew.c`, controller input and PIF sources | Fixes invalid pointer indirection, implicit-int declarations, a missing return, and the renamed runtime-method export setting; exports `HEAPU8` on `Module` for the host facade and exposes the framebuffer as an opaque pointer so Emscripten 6 can compile the frozen upstream core without weakening diagnostics. |
| `0005-emscripten-606-link-with-em++.patch` | `code/Makefile` | Links with `em++` instead of `emcc`. Emscripten **6.0.6** disabled `DEFAULT_TO_CXX` by default ([ChangeLog 6.0.6](https://github.com/emscripten-core/emscripten/blob/main/ChangeLog.md), #11121), so `emcc` no longer pulls in libc++ at link time and `wasm-ld` fails with `undefined symbol: std::__2::basic_string<…>::compare`. Only the link rule changes — the `%.o : %.cpp` / `%.o : %.c` compile rules stay on `emcc`, which still dispatches on file extension. |
