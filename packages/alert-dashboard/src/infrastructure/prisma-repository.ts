...
import {
  findObservedOccurrence,
  lockAlertFingerprints,
} from "#infrastructure/prisma-occurrence";
...
export function createPrismaRepository(
  ...
  return new PrismaAlertLedgerRepository(new PrismaClient({ adapter }));
}
...
export class PrismaAlertLedgerRepository implements AlertLedgerRepository {
  ...
  async ingestWebhook(input: IngestWebhookInput): Promise<IngestWebhookResult> {
    return this.#prisma.$transaction(async (transaction) => {
      ...
      await lockAlertFingerprints(
        transaction,
        input.payload.alerts.map((alert) => alert.fingerprint),
      );
      let opened = 0;
      let resolved = 0;
  ...
  async reconcileSnapshot(
    ...
    return this.#prisma.$transaction(
      async (transaction) => {
        ...
        let opened = 0;
        let resolved = 0;
        const absenceCandidates = await transaction.alertOccurrence.findMany({
          where: {
            lifecycleState: "open",
        ...
        await lockAlertFingerprints(transaction, [
          ...input.alerts.map((alert) => alert.fingerprint),
          ...absenceCandidates.map((occurrence) => occurrence.fingerprint),
        ]);
  ...
  async recordSnapshotFailure(
    ...
    await this.#prisma.snapshotRun.create({
      data: {
        id: SnapshotRunIdSchema.parse(crypto.randomUUID()),
        startedAtNs,
        completedAtNs,
