 ...
 *       from the shop to the customer. If the second check fails the charge is refunded.
 ...
 *       refunded and the items go back.
 * </ul>
 *
 * <p>Items only ever move on the main thread, after the ledger has answered. A refund the ledger
 * refuses is logged for staff; the items are never handed out twice to make up for it. Call {@link
 * #execute} on the main thread; the returned future completes on the main thread.
 */
public final class TradeEngine {
  ...
  /** Main thread, after the customer paid: move the goods, or refund if they are gone. */
  private CompletableFuture<TradeOutcome> deliver(Deal deal, Receipt receipt) {
    var problem = goodsProblem(deal);
    if (problem.isPresent()) {
      return refund(deal, receipt, problem.orElseThrow());
    }
  ...
  /** Main thread, after the shop paid: put the escrowed items in the shop, or undo both sides. */
  private CompletableFuture<TradeOutcome> stock(Deal deal, Receipt receipt) {
    ...
    if (room < deal.quantity()) {
      deal.customer().holdings().addOrDrop(deal.quantity());
      return refund(deal, receipt, new TradeProblem.ShopFull(room, deal.quantity()));
    }
  ...
  private CompletableFuture<TradeOutcome> refund(Deal deal, Receipt receipt, TradeProblem problem) {
    var reason = "refund:" + deal.reason();
    return wallets
        ...
        .handleAsync(
            (result, error) -> {
              ...
              return switch (result) {
                case Result.Ok<Receipt, EconomyError>(_) -> new TradeOutcome.Refunded(problem);
                case Result.Err<Receipt, EconomyError>(var refusal) ->
                    refundFailed(receipt, reason, problem, refusal.toString());
              };
            },
            mainThread);
  }
