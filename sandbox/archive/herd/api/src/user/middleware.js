...
async function authorizeAddUser(req, res, next) {
  if (!req.body.register) {
    ...
    authorizationMiddleware.checkUserIsAuthorized("app/users/*", "add")(
      req,
      res,
      next,
    );
...
async function authorizeGetUsers(req, res, next) {
  authorizationMiddleware.checkUserIsAuthorized("app/users/*", "get")(
    req,
    res,
    next,
  );
...
async function authorizeGetUser(req, res, next) {
  ...
  if (authenticatedUser._id === userToGet._id) {
    authorizationMiddleware.checkUserIsAuthorized("app/users/self", "get")(
      req,
      res,
      next,
    );
  } else {
    authorizationMiddleware.checkUserIsAuthorized(
      "app/users/" + userToGet._id,
      "get",
    )(req, res, next);
  }
}
...
async function authorizeUpdateUser(req, res, next) {
  ...
  let userToGet = res.locals.user;
  if (authenticatedUser._id === userToGet._id) {
    authorizationMiddleware.checkUserIsAuthorized("app/users/self", "update")(
      req,
      res,
      next,
    );
...
async function authorizeDeleteUser(req, res, next) {
  authorizationMiddleware.checkUserIsAuthorized("app/users/*", "delete")(
    req,
    res,
    next,
  );
