...
implementation detail, evidence trails, and future work in the existing
workflow taxonomy. The wiki publishes only exact workflow-document
...
- Keep `index.md` stable: do not add individual entries for `plans/` or `todos/`
...
- TODO docs may retain a durable `origin` and optional `source_marker: true`. Every source `TODO(todo:<id>)` marker MUST have a matching `<id>.md`; `bun run check-todos` validates this and the full Markdown model
...
with declared checks and an evidence requirement for each check. Use `runAt`
...
- **Keep `index.md` curated.** Stable categories can list individual docs; high-churn categories (`plans/`, `todos/`) should be directory links only. The Archive section in `index.md` should link to archive subdirectories, not individual archived files.
