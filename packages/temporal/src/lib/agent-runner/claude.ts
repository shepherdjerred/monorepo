...
import { providerSubprocessCommand } from "#shared/agent/agent-subprocess-identity.ts";
...
function queryOptions(input: RunClaudeAgentTurnInput): Options {
...
function handleMessage(input: {
  ...
  input.state.generationStarted ||=
    input.message.type === "assistant" || input.message.type === "result";
  input.state.possiblyAppliedEffects ||= messageMayApplyEffect(input.message);
