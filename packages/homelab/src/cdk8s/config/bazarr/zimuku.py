...
from .subhd import AI_LABEL, SIMPLIFIED_NAME, TRADITIONAL_NAME, episode_numbers, pick_archive_member, validate_content, MAX_DOWNLOAD, MAX_SUBTITLE, MAX_MEMBERS
...
class ZimukuProvider(Provider):
        ...
        for sub in subs_body.find_all("tr"):
            ...
            for language in language_list:
                ...
                subtitle.explicit_simplified = bool(SIMPLIFIED_NAME.search(a.text)) or any(
                    "china" in img.attrs["src"] and "hongkong" not in img.attrs["src"]
                    for img in sub.find("td", class_="tac lang").find_all("img")
                )
    ...
    def download_subtitle(self, subtitle):
        ...
        if rarfile.is_rarfile(archive_stream):
            ...
            if subtitle.language.alpha3 == "zho" and not (subtitle.language.country and subtitle.language.country.alpha2 == "TW"):
                ...
                if subtitle.target_season is not None and episode_numbers(decoded_filename + " " + subtitle.version) != {(subtitle.target_season, subtitle.target_episode)}:
...
def _get_subtitle_from_archive(archive, subtitle):
    if (subtitle.language.alpha3 == "zho" and not (subtitle.language.country and subtitle.language.country.alpha2 == "TW")
        ...
        name = pick_archive_member(archive.namelist(), subtitle.target_season,
                                   subtitle.target_episode, subtitle.target_release,
