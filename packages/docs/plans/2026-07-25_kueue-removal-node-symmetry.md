...
- Both workstreams implemented as commits on `feature/liskov-join` (PR
  #1629, still draft): `2c3399177` (talos symmetry + nodes.ts registry),
  `eb468bf1a` (full Kueue removal). `bun run verify -- --affected` green on
  each; `check:talos` validates both node pins; kueue sweep clean outside
  history docs.
- Docs groomed: restart-churn todo re-scoped to post-cutover verification
  (it was control-plane-wide, NOT kueue-only — deliberately not archived);
  capacity-remediation impl plan re-groomed around liskov; liskov join plan
  Phase 3 updated; buildkite-helper/helm-types-gen/talos-helper skills
  updated (chezmoi source + live copies).
- Reconciled with `temporal-worker[bot]` pushes to the PR branch (idle-gated
  crypto-mining alert + zpool runbook command) — rebased our commits on top
  of the bot's instead of force-pushing over them; one import conflict
  (ci-node → nodes rename) resolved.

### Remaining

- Merge happens on join day per the runbook (`src/talos/liskov/README.md`);
  post-merge live checks: Jobs not suspended, kueue-system gone, builds on
  liskov.
- Phase 3 (post-soak): torvalds relaxation + `BUILDKITE_MAX_IN_FLIGHT`
  revisit.

### Caveats

- The temporal-worker bot is actively pushing to this PR branch — always
  fetch + fast-forward before pushing; never force-push this branch.
- Kueue teardown ordering is atomic by design (label + controller + queues
  in one sync); do not cherry-pick parts of the kueue commit.
