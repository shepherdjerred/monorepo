    ...
    bets: [
      {
        ...
        isHouse: false,
        predictedTeamId: 100,
        submittedStake: BucksStakeSchema.parse(5),
        matchedStake: BucksAmountSchema.parse(5),
        unmatchedStake: BucksAmountSchema.parse(0),
        grossPayout: BucksAmountSchema.parse(10),
        houseCut: BucksAmountSchema.parse(1),
        payout: BucksAmountSchema.parse(9),
        winnings: BucksAmountSchema.parse(4),
    ...
    bets: [
      {
        ...
        stake: 5,
        grossPayout: BucksAmountSchema.parse(10),
        payout: BucksAmountSchema.parse(10),
        outcome: "won",
      },
    ],
  };
...
describe("refreshBucksMessages", () => {
  ...
  test("removes controls and shows exact final allocations after close", async () => {
    ...
    await addPosition({
      ...
      teamId: 200,
      stake: 1,
      matchedStake: BucksAmountSchema.parse(1),
      unmatchedStake: BucksAmountSchema.parse(0),
    });
    await addPosition({
      poolId: pool.id,
      ...
      stake: 4,
      isHouse: true,
      matchedStake: BucksAmountSchema.parse(4),
      unmatchedStake: BucksAmountSchema.parse(0),
    });
...
describe("announceSettlements unmatched receipts", () => {
  test("announces a fully unmatched offer without a settlement", async () => {
    ...
    await addPosition({
      ...
      teamId: 100,
      stake: 9,
      matchedStake: BucksAmountSchema.parse(0),
      unmatchedStake: BucksAmountSchema.parse(9),
    });
    await db.bucksBet.updateMany({
      ...
      data: {
        ...
        humanMatchedStake: 0,
        houseMatchedStake: 0,
        grossPayout: BucksAmountSchema.parse(0),
        fee: 0,
        payout: BucksAmountSchema.parse(0),
      },
    });
    const sends = await announcedMessages({
      closures: [
          ...
          positions: [
            {
              ...
              discordId: bucksTestDiscordId(1),
              teamId: 100,
              submittedStake: BucksStakeSchema.parse(9),
              matchedStake: BucksAmountSchema.parse(0),
              unmatchedStake: BucksAmountSchema.parse(9),
            },
          ],
          ...
          positions: [
            {
              ...
              submittedStake: BucksStakeSchema.parse(9),
              matchedStake: BucksAmountSchema.parse(1),
              unmatchedStake: BucksAmountSchema.parse(8),
            },
            {
              betId: 2,
              discordId: bucksTestDiscordId(2),
              teamId: 100,
              submittedStake: BucksStakeSchema.parse(1),
              matchedStake: BucksAmountSchema.parse(0),
              unmatchedStake: BucksAmountSchema.parse(1),
            },
          ],
        },
      ],
