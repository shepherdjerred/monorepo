...
fn test_create_dialog_focus_cycle() {
    ...
    // Simulate Tab cycling (backend-specific fields are skipped when not applicable)
    let focuses = [
...
async fn test_create_dialog_tab_navigation() {
    ...
    // Default backend is Zellij, so backend-specific fields are skipped
    handle_key_event(&mut app, key(KeyCode::Tab)).await.unwrap();
...
async fn test_create_dialog_toggle_backend() {
    ...
    // Test Right key (forward): Zellij → Docker → AiSandbox → Zellij
    ...
    handle_key_event(&mut app, key(KeyCode::Right))
        ...
        .unwrap();
    // Docker -> AiSandbox
    assert_eq!(app.create_dialog.backend, BackendType::AiSandbox);
...
async fn test_delete_blocked_during_create() {
    ...
    let (tx, _rx) = tokio::sync::mpsc::channel::<clauderon::tui::app::CreateProgress>(16);
...
async fn test_create_blocked_during_delete() {
    use clauderon::tui::app::CreateDialogFocus;
    ...
    let (tx, _rx) = tokio::sync::mpsc::channel::<clauderon::tui::app::DeleteProgress>(4);
...
fn test_directory_picker_opens_and_closes() {
    use clauderon::tui::app::DirectoryPickerState;
