...
Implemented by commit `8208b100e` / PR #1343. The Buildkite stack manages the
pipeline, cluster, default queue, and queue association. Agent-token management
was deliberately excluded and is documented in `cluster.tf`.

## Comment Log

- 2026-07-27 — Board audit verified `packages/homelab/src/tofu/buildkite/`
  contains the provider/backend and live resource definitions. Closed and
  archived as shipped; no unchecked work remains in this plan.
