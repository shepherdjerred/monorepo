...
> Phase 11 A/B experimentation framework were deleted from the repo in full — workflows,
...
In Progress — Phase 1 (foundation) implementation landed in PR #728; subsequent phases tracked via the team task list. Supersedes [`2026-04-25_pr-review-and-summary-bot.md`](../superseded/2026-04-25_pr-review-and-summary-bot.md), which should move to `archive/superseded/` once this plan is approved.

## Context

The monorepo's current AI code review surface is a soft-failing single-shot Dagger step (`scripts/ci/src/steps/code-review.ts` → `codeReviewHelper` in `.dagger/src/release.ts:853`) that posts unverified comments via `claude -p`. The earlier pending plan ([2026-04-25](../superseded/2026-04-25_pr-review-and-summary-bot.md)) sketched a Temporal webhook bot but predates the 2026 SOTA shift to multi-agent consensus + empirical verification (Cursor BugBot v11, Greptile v4, Copilot Code Review March 2026, Refute-or-Promote arxiv 2604.19049).
...
- **Metric rename per team-lead → align with eval's dashboard namespace** (drop the `_pipeline_` infix; "pipeline" is implied by `pr_review_`): `pr_review_pipeline_posted_total` → `pr_review_posted_total`, `pr_review_pipeline_findings_per_pr` → `pr_review_findings_per_pr`. Variable names match: `prReviewPostedTotal`, `prReviewFindingsPerPr`. Counter renamed away from the earlier `pr_review_count_total` choice after eval's drift review caught a name collision: eval's Task 8 owns `pr_review_count_total{repo, status=posted|skipped|failed}` for the **workflow-lifecycle** counter (fires on every run including kill-switch suppressions and pre-post failures). Our `pr_review_posted_total{owner, repo, outcome=created|updated}` is the **post-activity** counter (one tick per actually-posted comment, label distinguishes fresh-create vs edit-in-place). Different semantics, different labels, different names. Renaming now (before any series accumulates) avoids back-filling labels later and keeps the histogram aligned with the canonical plan's architecture diagram.
