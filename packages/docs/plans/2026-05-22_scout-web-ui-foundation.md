...
- CDK8s resource [`homelab/src/cdk8s/src/resources/scout/app.ts`](packages/homelab/src/cdk8s/src/resources/scout/app.ts) originally provisioned the Caddy + SPA Deployment, Service, and Cloudflare TunnelBinding (`scout-for-lol.com` for prod, `beta.scout-for-lol.com` for beta) with the Caddyfile served via ConfigMap and reverse-proxy rules for `/trpc/*` and `/api/*` to the scout backend
...
- **CF Tunnel ingress configuration**: confirm the cluster's CF Tunnel allows `scout-for-lol.com` (apex) and `beta.scout-for-lol.com`. The TunnelBinding is created with `disableDnsUpdates: true` because DNS for scout-for-lol.com is managed in OpenTofu — verify that pointer still routes correctly.
- **Add OAuth redirect URI** in the Discord Developer Portal for both apps: `https://scout-for-lol.com/api/auth/discord/callback` (prod) and `https://beta.scout-for-lol.com/api/auth/discord/callback` (beta).
