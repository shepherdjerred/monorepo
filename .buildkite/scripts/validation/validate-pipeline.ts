...
import { summarySteps } from "../images/build-summary-tables.ts";
...
for (const required of [
  ...
  "bun --no-install .buildkite/scripts/selectors/select-main-pipeline.ts",
...
const ciChanged = await Bun.file(
  ".buildkite/scripts/selectors/ci-changed.ts",
).text();
...
for (const selectorInput of [
  ...
  ".buildkite/scripts/images/bake-images.ts",
...
// Bun's global cache.
const bakeImages = await Bun.file(
  ".buildkite/scripts/images/bake-images.ts",
).text();
...
assertNoImplicitBunRuntime([
  ...
  { path: ".buildkite/scripts/selectors/ci-changed.ts", source: ciChanged },
  { path: ".buildkite/scripts/images/bake-images.ts", source: bakeImages },
  {
    path: ".buildkite/scripts/selectors/select-main-pipeline.ts",
    source: mainSelector,
  },
]);
