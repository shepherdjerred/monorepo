...
1. Establish the selected account with `linear auth whoami`.
2. Resolve an exact issue identifier with `linear issue id`, a user-provided
...
`linear api` is a raw escape hatch, not permission to issue broad mutations.
Before using it, inspect `linear api --help` and `linear schema --help`, then
