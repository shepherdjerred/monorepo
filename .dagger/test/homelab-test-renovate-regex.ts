    ...
    try {
      ...
      for (const entry of entries) {
        if (!entry.hasRenovateComment && !entry.isExcluded) {
          console.log(
            `❌ ${entry.property} (line ${String(entry.line)}): Missing Renovate comment`,
          );
          fileErrors++;
      ...
      console.log(
        `   Excluded from Renovate: ${String(entries.filter((e) => e.isExcluded).length)}`,
      );
