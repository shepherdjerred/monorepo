...
`packages/voice-assistant` (`@shepherdjerred/voice-assistant`), with the per-session state machine
in `src/audio-lifecycle.ts` and the cloud turn in `src/realtime-turn.ts`. Discord transports,
prom-client metrics, logging, OTel span prefixes, the wake phrase, and the playback tools are all
injected ports, so streambot's series and trace names survived the extraction byte-identical.
Streambot's own bindings and composition stay in `packages/streambot/src/voice/`: the
hey-streambot constants and asset manifest (`constants.ts`), the model-loading and lifecycle-port
bindings (`local-voice.ts`), the production Realtime turn binding (`realtime-voice.ts`), the
playback tools (`voice-tools.ts`), and the session composition root
(`voice-assistant-session.ts`), with the metric adapters in
`packages/streambot/src/observability/voice-metrics-ports.ts`.
...
- The utterance ends locally and both it and the whole cloud transaction are capped. Cloud
  ...
  `cloud-verification-rate-limiter.ts`, with a cooldown after a rejected transcript. Working PCM and verifier features are erased after
...
`local-models.ts` (loading the manifest streambot builds in `constants.ts`) treats every one of
those as fatal rather than degrading to a weaker gate.
