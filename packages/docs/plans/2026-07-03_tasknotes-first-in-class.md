...
- **Contract test** `src/data/api/__tests__/contract.test.ts`: `Bun.spawn` the real server (`cwd ../tasknotes-server`, temp vault, `PORT=8791`), poll `/api/health`, run the real `TaskNotesClient` method matrix + error paths (404→NotFoundError, bad token→ApiError). Pins the _current_ contract; later keeps the P3 legacy adapter honest. CI: new Dagger fn `tasknotesContractTest` in `.dagger/src/misc.ts` + Buildkite step in `scripts/ci/src/steps/per-package.ts` (trigger on either package).
