...
- **CORRECTION (2026-07-12): the Arena "0 rows" was NOT data sparsity — it was a queue-ID misclassification bug, and this caveat was itself corrupted by that bug.** The "zero tracked arena rows in the last 30 days / Dan + Danny only 7 shared games" figures were measured off the mislabeled lake: Riot moved live Arena from queue `1700` → `1750` (both `gameMode: "CHERRY"`) around 2026-05-13, but `parseQueueType` only mapped `1700`, so every Arena game since was flattened with `queue = null` and excluded from `queue IN ('arena')`. The validation snapshot therefore only "saw" the pre-2026-05-13 `1700` games. Real numbers over the true Arena data: **424 beta Arena games (107 in the last 30 days)**, and Dan + Danny have **80 shared-subteam games (30 in the last 30 days)** — well past the `games >= 10` floor. The subteam fix is still correct and independent; the emptiness was the queue label. Fixed in the `2026-07-12_scout-s3-canonical-raw-store` plan (Part 1: `flatten.ts` now classifies via `resolveQueueTypeFromGame`, which keys off `CHERRY` mode).
...
- PR #1457 merged subscription mute support, UI polish, and generalized player grouping.
