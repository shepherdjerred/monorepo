...
| `CNAME` bugsink.shepherdjerred.com.sjer.red | cfargotunnel (+ `_managed.bugsink...sjer.red` TXT) |
...
| `TXT` \_externaldns.srv-\_minecraft.\_tcp.ts-mc.net | Stale ownership TXT. No DNSEndpoint defines this SRV anymore. |
...
**better-skill-capped.com:** CNAME (apex), TXT (SPF), TXT (\_dmarc), TXT (_.\_domainkey)
-- all 4 records are unmanaged despite k8s defining \_dmarc and_.\_domainkey
...
2. Split into separate DNSEndpoints so the CNAME is in its own resource
   (won't help -- external-dns merges all endpoints before conflict detection)
