...
- Produced a self-contained Codex CLI-only controller and worker prompt.
- Reduced the prior complete copy from 4,411 words to 1,320 words (70.1%) while
  preserving the readiness contract.
- Replaced portable scheduler/process fallbacks with Codex CLI Goal,
  `spawn_agent`/`followup_task`/`interrupt_agent`, and `wait_agent`.
- Removed desktop/web Scheduled-task concepts and the redundant Goal-creation
  branch; the prompt is entered directly through `/goal`.
- Kept current-head Buildkite, GraphQL review-thread, and direct merge-tree
  readiness while delegating repository engineering policy to `AGENTS.md`.
- Left the existing Claude Code prompt and configuration unchanged.
- Ran five independent read-only subagent reviews across CLI semantics,
  liveness, readiness evidence, worktree safety, and prompt quality.
- Verified the highest-impact findings against the current Codex manual,
  repository Buildkite/code-review implementation, shared git-spice ref, and
  current `wait_agent` behavior.
- Replaced the oversized inline Goal with a short `/goal <objective>` and a
  versioned, Codex CLI-only durable controller contract.
- Added controller fencing, atomic state, finite command deadlines,
  parameterized current-head evidence, exclusive worktree use, repository-wide
  git-spice serialization, typed worker outcomes, and verified shutdown.

### Remaining

- Re-run the five review lenses before any live Goal test.

### Caveats

- No Goal, worker, PR, branch, Buildkite job, or hosted review was started in
  this prompt-design session.
- The revised contract has not yet been subjected to a second five-agent review
  or a live Goal test.
- The generic `pr-monitor` and `pr-health` skills contain stale assumptions
  about review provider, approval, polling, conflict detection, and staging.
  The proposed prompt explicitly treats them as non-authoritative.
- The proposed prompt is not safe to run yet; the review found blocking Goal
  invocation, state/lease fencing, evidence-binding, and worktree/stack issues.
