...
categories.forEach((cat, i) => {
  console.log(`  ${i + 1}. ${cat.category}: ${cat.count} transactions ($${cat.total?.toFixed(2) ?? '0'})`);
});
console.log();

// Top merchants
console.log("🏪 Top 15 merchants:");
const merchants = db.query(`
...
merchants.forEach((m, i) => {
  console.log(`  ${i + 1}. ${m.merchant}: ${m.count} transactions ($${m.total?.toFixed(2) ?? '0'})`);
});
console.log();
...
accounts.forEach((acc, i) => {
  console.log(`  ${i + 1}. ${acc.account}: ${acc.count} transactions ($${acc.total?.toFixed(2) ?? '0'})`);
});
