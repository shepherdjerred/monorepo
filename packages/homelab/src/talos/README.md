...
- `systemReserved.memory: 24Gi` - Non-pod memory reserved for ZFS ARC (16 GiB) plus host/kernel/kubelet/containerd overhead (~8 GiB). Right-sized from 56Gi on 2026-07-10 when pod requests hit 99.99% of allocatable and CI pods went unschedulable; see `packages/docs/plans/2026-07-10_torvalds-memory-rightsize.md`.
