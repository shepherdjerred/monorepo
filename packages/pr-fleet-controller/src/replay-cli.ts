...
if (parsed.values.json) {
  ...
  process.stdout.write(
    `replay verified run=${report.runId} status=${report.status} events=${String(report.eventCount)} commands=${String(report.commands.completed)}/${String(report.commands.started)} tools=${String(report.tools.completed)}/${String(report.tools.started)} workers=${String(report.workers.completed)}/${String(report.workers.started)} workerAttempts=${String(report.workerAttempts.completed)}/${String(report.workerAttempts.started)} masterTurns=${String(report.masterTurns.completed)}/${String(report.masterTurns.started)}\n`,
  );
}
