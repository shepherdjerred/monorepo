    ...
    <nav className="scout-app-sidebar-nav" aria-label="App navigation">
        ...
        <section className="space-y-0.5 shrink-0" aria-label="Tools">
          ...
          {tools.map((item) => (
            <NavLink
              ...
              className="scout-app-sidebar-link flex items-center gap-2 px-2 py-1.5 text-xs"
            >
              {item.to === "/explore" && (
                <Compass className="size-3.5 shrink-0 text-scout-subtle" />
              )}
              {item.to === "/players" && (
                <Users className="size-3.5 shrink-0 text-scout-subtle" />
              )}
              {item.to === "/bucks" && (
                <Coins className="size-3.5 shrink-0 text-scout-subtle" />
              )}
        ...
        >
          <div className="flex items-center justify-between px-2 pb-1 shrink-0">
            ...
            <button
              ...
              title="New chat"
              aria-label="New chat"
              className="flex size-5 items-center justify-center rounded text-scout-subtle transition-colors hover:bg-scout-hover hover:text-scout-ink"
              onClick={() => {
                void navigate("/explore");
              }}
            >
            ...
            {guildItems.map((item) => {
              ...
              return (
                <NavLink
                  ...
                  className="scout-app-sidebar-link flex items-center gap-2 px-2 py-1.5 text-xs"
                >
                  <Icon className="size-3.5 shrink-0 text-scout-subtle" />
                  <span>{item.label}</span>
                </NavLink>
              );
            })}
      ...
      >
        <NavLink
          ...
          className="scout-app-sidebar-link flex items-center gap-2 px-2 py-1.5 text-xs"
        >
          <Settings className="size-3.5 shrink-0 text-scout-subtle" />
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <button
              type="button"
              className="group flex w-full items-center justify-between gap-2 rounded-lg px-2 py-1.5 text-left text-xs font-medium text-scout-ink transition-colors hover:bg-scout-hover"
            >
              <div className="flex min-w-0 items-center gap-2">
                <div className="flex size-6 shrink-0 items-center justify-center rounded-md border border-scout-border/60 bg-scout-canvas text-[11px] font-semibold text-scout-ink">
                  {selectedGuild ? (
                    selectedGuild.name.slice(0, 2).toUpperCase()
                  ) : (
                    <Server className="size-3 text-scout-subtle" />
                  )}
                </div>
                <div className="min-w-0 flex-1">
                  <p className="truncate text-xs font-medium leading-tight text-scout-ink">
                    {selectedGuild ? selectedGuild.name : "Switch server"}
                  </p>
                  <p className="truncate text-[10px] leading-tight text-scout-subtle">
                    {selectedGuild ? "Active server" : "No server selected"}
                  </p>
                </div>
              </div>
              <ChevronsUpDown className="size-3.5 shrink-0 text-scout-subtle transition-colors group-hover:text-scout-ink" />
            </button>
          </DropdownMenuTrigger>
          <DropdownMenuContent align="start" className="w-56">
            ...
            {guildsQuery.data?.map((guild) => (
              ...
              >
                <div className="flex min-w-0 items-center gap-2">
                  <div className="flex size-5 shrink-0 items-center justify-center rounded border border-scout-border/60 bg-scout-canvas text-[10px] font-semibold text-scout-ink">
                    {guild.name.slice(0, 2).toUpperCase()}
                  </div>
                  <span className="truncate">{guild.name}</span>
                </div>
                {guild.id === guildId && (
                  <Check className="ml-1 size-3.5 shrink-0 text-scout-primary" />
                )}
              </DropdownMenuItem>
            ))}
          </DropdownMenuContent>
