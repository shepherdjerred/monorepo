...
async fn test_sprites_auto_destroy_false_persists() {
    ...
    match result {
        Ok(returned_name) => {
            ...
            // Now actually clean up using a backend with auto_destroy=true
            let mut cleanup_config = SpritesConfig::load_or_default();
            cleanup_config.lifecycle.auto_destroy = true;
            let cleanup_sprites = SpritesBackend::with_config(cleanup_config);
            let _ = cleanup_sprites.delete(&returned_name).await;
        }
        Err(e) => {
            eprintln!("Sprite creation failed: {e}");
        }
    ...
    match result {
        Ok(returned_name) => {
            ...
            match (output_5, output_20) {
                ...
                (Err(e5), _) => {
                    eprintln!("get_output(5) failed: {e5}");
                }
                (_, Err(e20)) => {
                    eprintln!("get_output(20) failed: {e20}");
                }
            }
...
async fn test_sprites_parallel_creation() {
    ...
    if let Ok(name) = result1 {
        ...
        eprintln!("Sprite 1 creation failed: {:?}", result1.err());
    }

    if let Ok(name) = result2 {
        created_names.push(name);
    } else {
        eprintln!("Sprite 2 creation failed: {:?}", result2.err());
    }

    if let Ok(name) = result3 {
        created_names.push(name);
    } else {
        eprintln!("Sprite 3 creation failed: {:?}", result3.err());
    }
    ...
    println!(
        "Successfully created {} sprites in parallel",
        created_names.len()
    );
