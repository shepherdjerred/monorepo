...
const README_PR_BODY =
  "Automated README updates generated by cog + Codex CLI in Dagger.";
...
`;
  ...
  const container = dag
    ...
    .withMountedCache(
      "/var/cache/apt",
      dag.cacheVolume(`apt-cache-bun-${BUN_VERSION}-readme`),
    )
    .withMountedCache(
      "/var/lib/apt",
      dag.cacheVolume(`apt-lib-bun-${BUN_VERSION}-readme`),
    )
