...
export function versionCommitBackStep(dependsOn: string[]): BuildkiteStep {
  ...
  return daggerStep({
    ...
    daggerCmd: `bash .buildkite/scripts/collect-digests.sh /tmp/digests.json ${keyArgs} && dagger call version-commit-back --digests "$(cat /tmp/digests.json)" --version "2.0.0-$BUILDKITE_BUILD_NUMBER" --gh-token 'env:GH_TOKEN?nonce=$BUILDKITE_BUILD_NUMBER'${DRYRUN_FLAG}`,
    timeoutMinutes: 10,
    condition: MAIN_ONLY,
    dependsOn,
    priority: 1,
