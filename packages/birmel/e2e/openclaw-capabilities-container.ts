...
function getExecutableTool(tool: unknown): ToolLike {
  ...
  return { execute: async (input) => await Reflect.apply(execute, undefined, [input]) };
}
  ...
  return Bun.serve({
    ...
    fetch(request) {
      ...
      if (url.pathname === "/instances/instance-1/tabs") {
        return Response.json([{ id: "tab-1", url: "http://localhost:9867/page" }]);
      }
...
async function setupPhase(): Promise<void> {
  ...
  expectSuccess(
    await sessionTool.execute({
      action: "steer",
      guildId: "guild-1",
      sessionId: (await prisma.agentSession.findFirstOrThrow()).id,
...
async function verifyPhase(): Promise<void> {
  ...
  if (runs.length === 0 || runs[0]?.status !== "success") {
    throw new Error(`Expected successful persisted job run, got ${JSON.stringify(runs)}`);
  }
}
