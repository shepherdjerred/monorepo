...
export function buildPrompt(goal: string, context: PromptContext): string {
  return [
    ...
    "- pokemonctl observe — preferred authoritative compact state; add --full for detailed readiness/collision/game data or --screenshot when pixels matter.",
    "- pokemonctl tap <button> [--repeat n] — preferred atomic menu or battle input.",
    "- pokemonctl move <north|south|west|east> [--tiles n] — preferred verified overworld movement.",
    "- pokemonctl interact [north|south|west|east|ahead] — preferred guarded adjacent interaction.",
    "- pokemonctl advance — one safe scripted-dialog step; repeat only after observing dialogInputReady.",
    "- pokemonctl wait --until <ready|stable|phase-change> [--timeout-ms n] — wait for evidence, not a guessed delay.",
    "- pokemonctl map show [--radius n] — inspect the local collision grid.",
    "- pokemonctl navigate --x n --y n --max-steps n — bounded travel to known coordinates on the current map only. Always use a finite step budget, then re-observe and replan on any context change.",
    "- Raw pokemonctl press and pokemonctl chord are escape hatches only when the semantic controls cannot express the next atomic action. Never issue a blind long chord.",
    "- pokemonctl wait --seconds n — compatibility delay for animations that expose no readiness signal.",
    ...
    "- Prefer semantic observe/tap/move/interact/advance/wait/map/navigate controls. They return before/after evidence and stop on context changes.",
    "- Observe after every action. Take and read a screenshot when menus, dialog, battle visuals, or landmarks require pixels.",
