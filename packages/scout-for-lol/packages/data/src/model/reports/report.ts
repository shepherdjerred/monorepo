...
import {
  ...
  ReportTitleSchema,
} from "#src/model/core/form-inputs.ts";
import {
  CompetitionCronSchema,
  DEFAULT_SCHEDULE_TIMEZONE,
  ReportScheduleTimezoneSchema,
} from "#src/model/competitions/competition-cron.ts";
import type { CompetitionId } from "#src/model/competitions/competition.ts";
import type {
  DiscordAccountId,
  DiscordChannelId,
  DiscordGuildId,
...
export const ReportChartStackSchema = z.enum(["none", "normal", "percent"]);
export const ReportRollingTransformSchema = z
  .object({ window: z.number().int().min(2).max(52) })
  .strict();
...
export const ReportChartOptionsSchema = z
  .object({
    ...
    rolling: ReportRollingTransformSchema.optional(),
    cumulative: z.boolean().optional(),
    stack: ReportChartStackSchema.optional(),
  ...
  .superRefine((options, ctx) => {
    if (options.rolling !== undefined && options.cumulative === true) {
      ctx.addIssue({
        code: "custom",
        path: ["rolling"],
        message: "Rolling and cumulative transforms are mutually exclusive.",
      });
    }
  });
...
export type ReportTableOptions = z.infer<typeof ReportTableOptionsSchema>;
export const ReportTableOptionsSchema = z
  .object({ sparkline: z.boolean().optional() })
  .strict();
...
export const ReportRenderSpecSchema = z.discriminatedUnion("kind", [
  ...
  z.object({
    ...
    options: ReportTableOptionsSchema.optional(),
  }),
  z.object({
...
export type ReportRun = {
  ...
  visualizationS3Key: string | null;
  visualizationByteSize: number | null;
  createdAt: Date;
};
