...
export type SendOptions = {
  signal?: AbortSignal;
  /** Attachments staged by a tool during the turn, e.g. a generated image. */
...
export async function handleSend(
  ...
  const signal = options.signal ?? NON_ABORTING_SIGNAL;
  ...
  const result = await withSendableChannel<Message>(
    ...
    async (channel) => {
      ...
      return await channel.send(
        files.length === 0 ? content : { content, files: [...files] },
      );
    },
  );
  if (!result.ok) {
