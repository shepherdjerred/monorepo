...
function setEnvironment(value: string | undefined): boolean {
  return value === undefined
    ? Reflect.deleteProperty(Bun.env, ENV_NAME)
    : Reflect.set(Bun.env, ENV_NAME, value);
}

afterEach(() => {
  setEnvironment(ORIGINAL_VALUE);
});
