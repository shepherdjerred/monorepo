...
export function versionCommitBackStep(dependsOn: string[]): BuildkiteStep {
  return daggerStep({
    ...
    daggerCmd: `dagger call version-commit-back --digests "$(buildkite-agent meta-data get image-digests)" --version "$(buildkite-agent meta-data get release-version || echo dev)" --gh-token env:GITHUB_TOKEN${DRYRUN_FLAG}`,
    timeoutMinutes: 10,
    condition: MAIN_ONLY,
    dependsOn,
  });
