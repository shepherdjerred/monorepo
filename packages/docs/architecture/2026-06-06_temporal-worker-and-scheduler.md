...
- **Schema** — new submissions use `AgentTaskInputV2`: `contractVersion: 2`, `title`, `prompt`, `provider` (`claude`|`codex`), `repo.fullName`, and one or more declared `checks` with required/optional status plus an evidence requirement. Every check has independently executed collectors, and every collector declares a source-defined expectation over accepted exit codes, typed JSON paths, or numeric Prometheus samples. `mode` is `report-only` (the only value). `runAt` and `cron` are mutually exclusive; recurring inputs use a stable `scheduleId`. The v1 shape remains only for Temporal history replay.
...
- **Workflow** — `agentTaskWorkflow` (`workflows/agent-task.ts`): server-side delay → workdir clone → investigation subprocess → provider evidence-receipt extraction and redaction → independent declared collection and predicate evaluation → finalization subprocess over that explicit receipt catalog → strict result validation → shared delivery → optional report-only follow-up → cleanup. Finalization cannot use Claude tools, and evidence gathered during finalization is never accepted. Each declared check is normalized independently. Missing, failed, skipped, unknown, uncaptured, or unevaluated evidence forces partial execution. Adverse evidence keeps a successfully collected run complete but overrides a model-authored pass and produces attention. Agents may emit a retirement recommendation, but cannot pause or cancel schedules. A generic failure report is accepted before the workflow rethrows so Temporal still records the failure.
