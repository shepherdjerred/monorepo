...
realtime turn completes only after every observed function call reaches
`agent_tool_end` and a later SDK `agent_end` arrives. It never completes on
`audio_stopped`, and it accepts the `conversation.item.added` GA event name.
The fake-transport unit tests cannot catch real-API drift;
`e2e/voice-receive-repro.ts` (transport) and `voice:cloud-probe` (cloud turn)
are the real-API oracles.
