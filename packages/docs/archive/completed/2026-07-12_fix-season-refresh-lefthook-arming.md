...
`scout-season-refresh-weekly-workflow-2026-07-20T14:00:00Z` completed in 5m16s
and opened PR #1596. That run exercised the Claude edit, hook disarm, commit,
push, and PR path, directly proving the lefthook regression fixed.

The 2026-07-27 failure occurred inside `claude -p`, before git commit, while the
deployed worker still used a deprecated model. It is not a recurrence of this
bug and is tracked in
`packages/docs/todos/scout-season-refresh-opus5-production-verification.md`.
