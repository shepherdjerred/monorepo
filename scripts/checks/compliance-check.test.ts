...
import { isNoopScript } from "../misc/migration-core.ts";
...
test("Turbo compliance inputs cover deeply nested workspace manifests", async () => {
  ...
  try {
    ...
    await Bun.write(`${root}/scripts/checks/compliance-check.ts`, "");

    const turboConfig = TurboConfigSchema.parse(
      Bun.JSONC.parse(
        await Bun.file(
          path.join(import.meta.dir, "..", "..", "turbo.json"),
        ).text(),
      ),
    );
    ...
    expect(
      ...
      "scripts/checks/compliance-check.ts",
...
describe("invokesAmbiguousTypeScriptCompiler", () => {
  test.each([
    ...
    "bun --if-present x tsc --noEmit",
    "bun --if-present --silent x tsc --noEmit",
    "bun --future-unknown-flag x tsc --noEmit",
    "bun run tsc --noEmit",
    "bun run --silent tsc --noEmit",
    "bun --if-present run tsc --noEmit",
    "env tsc --noEmit",
    "env NODE_OPTIONS=--max-old-space-size=4096 tsc --noEmit",
    "env -i NODE_OPTIONS=4096 tsc --noEmit",
    "env -u FOO NODE_OPTIONS=4096 tsc --noEmit",
    "/usr/bin/env tsc --noEmit",
    "env NODE_OPTIONS=4096 bun x tsc --noEmit",
    "env NODE_OPTIONS=4096 bun run tsc --noEmit",
    "env -S 'tsc --noEmit'",
    "env -S 'NODE_OPTIONS=x tsc'",
    "env --split-string='bun x tsc --noEmit'",
    "env -S 'bun run tsc --noEmit'",
    "env -S'tsc --noEmit'",
    "env -S'NODE_OPTIONS=x tsc'",
    "env -S'bun x tsc --noEmit'",
    "env -S'bun run tsc --noEmit'",
  ...
  test.each([
    ...
    "env PATH=node_modules/@typescript/native/bin:$PATH tsc --noEmit",
    "env -S 'PATH=node_modules/@typescript/native/bin:$PATH tsc --noEmit'",
    "env -S'PATH=node_modules/@typescript/native/bin:$PATH tsc --noEmit'",
