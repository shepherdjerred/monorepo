...
import "@shepherdjerred/scout-backend/metrics/index.ts";

// Initialize HTTP server for health checks and metrics
logger.info("🌐 Starting HTTP server for health checks and metrics");
import { shutdownHttpServer } from "@shepherdjerred/scout-backend/http-server.ts";

logger.info("🔌 Starting Discord bot initialization");
import "@shepherdjerred/scout-backend/discord/index.ts";

logger.info("⏰ Starting cron job scheduler");
import { startCronJobs } from "@shepherdjerred/scout-backend/league/cron.ts";
startCronJobs();
