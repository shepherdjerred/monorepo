...
- **Licensed Berkeley Mono bootstrap:** `install_macos.sh` discovers the externally supplied static TTF package under `~/Downloads` (or `BERKELEY_MONO_SOURCE_DIR`) and invokes `packages/fonts/patch-berkeley-mono.py`; source and patched fonts must remain outside the repository
