...
- **No CI** (pipeline removed 2026-07): the stub keeps `bun run typecheck` green anywhere without HA credentials; strict typing requires a local `bun run generate` against live HA.
...
- **`rootInstallWithoutHooks(repoDir)`** — root `bun install --frozen-lockfile --ignore-scripts`. Bot clones are **not dev checkouts**: historically a plain root install ran the root `prepare` script (`lefthook install`), arming the full dev pre-commit suite for the bot's later `git commit` inside the worker pod, where it couldn't pass (no gitleaks binary, no per-package toolchains) — this exact mistake broke `scout-season-refresh-weekly` and `readme-refresh-weekly` every week through June–July 2026. lefthook was removed from the repo 2026-07, but keep using the hook-free install: it's faster and there is no CI on the bot's PR anymore, so a human reviewer is the gate.
...
The **`scripts/rehearse-bot-clone.ts`** rehearsal script drives these same helpers plus canaries for the cog targets and the hook-free commit path. It used to run automatically as the `temporal-schedule-rehearsal` CI step on every build touching temporal; that pipeline was removed 2026-07, so **run it manually** (inside the worker image, against your branch's tree) before merging a change to scheduled activities — otherwise a break surfaces silently on the weekend. If you add a new repo-path or install-step dependency to a scheduled activity, extend the rehearsal script in the same PR.

## Greptile review threads (historical CI gate)

A `greptile-review` Buildkite step used to gate `ci-complete` for PRs; it was removed 2026-07 with the pipeline. The thread-evaluation knowledge below still applies to any bot (e.g. pr-babysit) that reasons about Greptile reviews — separate from the in-package PR review bot below.
