 ...
 *
 * The agent creates one pod per pipeline step and a workspace claim per
 * workflow, then streams logs back. It is deliberately namespaced: nothing it
 * does should reach outside the CI namespace, and step pods themselves run
 * under the tokenless `woodpecker-job` account with no API access at all.
 */
    ...
    withCommonProps({
      ...
      envVariables: {
        ...
        WOODPECKER_BACKEND_K8S_NAMESPACE:
          EnvValue.fromValue(WOODPECKER_NAMESPACE),
        ...
        // Per-workflow workspace claims land on the CI node's NVMe.
