...
export function QuestionView({ message }: QuestionViewProps) {
  ...
  const questionTool = message.toolUses?.find(
  ...
  return (
    <div className="border-4 border-accent bg-accent/5 p-6 space-y-6">
      ...
      <div className="border-t-2 border-accent/30 pt-3">
        <p className="text-xs font-mono text-muted-foreground">
          📝 This question was presented in the active Claude Code session.
          Check the terminal for response status.
        </p>
      </div>
    </div>
  );
...
export function isQuestion(message: Message): boolean {
  return (
    message.toolUses?.some((tool) => tool.name === "AskUserQuestion") ?? false
  );
}
