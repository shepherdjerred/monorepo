...
- **Prod (explicit promotion):** `AWS_PROFILE=seaweedfs bun scripts/promote-scout.ts` opens one PR moving BOTH `versions.ts` pins together — `scout-for-lol-site/prod` (site artifact version) and `shepherdjerred/scout-for-lol/prod` (the beta image line copied verbatim; the `2.0.0-<n>` in an image pin is a cosmetic label on a digest-pinned ref). On merge, ArgoCD deploys the backend and the `scout-prod-reconcile` CI step syncs the prod bucket from the archived artifact. Rollback = revert the promotion commit.
