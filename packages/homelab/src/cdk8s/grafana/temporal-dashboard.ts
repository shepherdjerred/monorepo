  ...
  return {
    ...
    panels: [
      ...
      timeseriesPanel({
        ...
        targets: [
          {
            expr: 'sum by (namespace, workflowType, activityType) (increase(activity_task_fail{namespace=~"prod|beta"}[1h])) or on() vector(0)',
            legend: "{{namespace}} {{workflowType}} {{activityType}}",
          },
        ],
        x: 0,
        y: 12,
