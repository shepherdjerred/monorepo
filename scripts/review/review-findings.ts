 ...
 *
 * Usage:
 *   GH_TOKEN=$(gh auth token) bun scripts/review/review-findings.ts list 2104
 *   GH_TOKEN=$(gh auth token) bun scripts/review/review-findings.ts dismiss 2104 \
 *     --finding "S3 fetch lacks timeout" --reason "fixed in 10224eabd"
 *   GH_TOKEN=$(gh auth token) bun scripts/review/review-findings.ts resolve-thread 2104 \
...
async function listCommand(
  ...
  console.log(
    `\nFix and re-request a review, or — once verified fixed or incorrect — ` +
      `dismiss with:\n  bun scripts/review/review-findings.ts dismiss ${String(prNumber)} --finding "<title>" --reason "<why>"`,
  );
}
...
function usage(): never {
  console.error(
    [
      "Usage:",
      "  bun scripts/review/review-findings.ts list <pr>",
      '  bun scripts/review/review-findings.ts dismiss <pr> --finding "<title>" --reason "<why>"',
      '  bun scripts/review/review-findings.ts resolve-thread <pr> --thread <id> --reason "<why>"',
      "",
