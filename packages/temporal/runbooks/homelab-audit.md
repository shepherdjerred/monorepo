...
> Kubernetes workload health, ArgoCD state, and CI main failures with
> failed-step logs. Use this runbook when those signals require a deeper human-
...
| `toolkit woodpecker` | `WOODPECKER_URL`, `WOODPECKER_TOKEN`, `WOODPECKER_REPO_ID` | `toolkit woodpecker pipeline ls $WOODPECKER_REPO_ID --limit 1` |
...
in Linear, not part of this recurring report-only audit. Keep that issue open
until the canary and seven consecutive daily audits pass.

## Section 11: CI on `main`

Woodpecker is the source of truth for CI; this repo does not use GitHub Actions. Requires `WOODPECKER_URL`, `WOODPECKER_TOKEN`, and `WOODPECKER_REPO_ID` in env.

### Is `main` green?

```bash
toolkit woodpecker pipeline ls "$WOODPECKER_REPO_ID" --branch main --limit 5   # last 5 pipelines on main
toolkit woodpecker pipeline show "$WOODPECKER_REPO_ID" <PIPELINE>              # workflows and step ids
toolkit woodpecker pipeline log show "$WOODPECKER_REPO_ID" <PIPELINE>          # failed step logs
```

Flag: latest `main` pipeline `failure` / `error` / `killed`, or a sustained streak of failures (a red `main` means deploys are stale — the last-green image is still running).

### In-flight pipelines

```bash
toolkit woodpecker pipeline queue                                              # pending and running work
toolkit woodpecker pipeline ls "$WOODPECKER_REPO_ID" --limit 20                # recent pipelines and status
```

Flag: pipelines running >60 min (stuck), pending pipelines piling up (agent starvation — cross-check the agent pool below).

### Agent pool

```bash
kubectl get pods -n woodpecker                            # server, agent, extension, step pods
toolkit woodpecker info                                   # server and connected-agent view
```

Flag: agent pod not Running, zero connected agents while pipelines are pending. The `WoodpeckerAgentDisconnected` alert covers the same condition.
...
toolkit woodpecker pipeline log show "$WOODPECKER_REPO_ID" <PIPELINE>  # CI logs for the failing check
```
...
| `woodpecker`                   | `woodpecker`                   | Server, agent, config extension, and Postgres ready; connected agents exist when pipelines are pending; large Error/NotReady step pod buildup is Yellow unless current pipelines are blocked, then Red. |
...
| `dagger`                       | `dagger`                       | Dagger engine StatefulSet ready; Section 11 CI steps can start Dagger work; engine restart loops or stuck CI steps are Red.                                                                             |
...
| `minecraft-sjerred`            | `minecraft-sjerred`            | Desired replicas currently `0` is Green/idle; if scaled up, StatefulSet ready, Minecraft/RCON reachable, and Dynmap ingress proxy ready.                                                                |
...
- PR `statusCheckRollup` failures on CI checks should match `toolkit woodpecker pipeline ls` output for that commit; a mismatch means GitHub's check status is stale and a re-run may be needed
