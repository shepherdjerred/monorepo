...
export const manageBirthdayTool = createTool({
  id: "manage-birthday",
  description: "Manage birthdays: set, get, update, delete, get today's, get upcoming, or get by month",
  inputSchema: z.object({
    guildId: z.string().describe("The Discord guild ID"),
    action: z.enum(["set", "get", "update", "delete", "today", "upcoming", "by-month"]).describe("The action to perform"),
    userId: z.string().optional().describe("User ID (for set/get/update/delete)"),
    ...
    daysAhead: z.number().min(1).max(365).optional().describe("Days to look ahead (for upcoming)"),
    month: z.number().min(1).max(12).optional().describe("Month to query (for by-month)"),
  }),
        ...
        switch (ctx.action) {
          case "set": {
            if (!ctx.userId || !ctx.birthMonth || !ctx.birthDay) {
              return { success: false, message: "userId, birthMonth, and birthDay are required for set" };
            }
            ...
            logger.info("Birthday set", { guildId: ctx.guildId, userId: ctx.userId });
            return {
              success: true,
          ...
          case "update": {
            ...
            logger.info("Birthday updated", { guildId: ctx.guildId, userId: ctx.userId });
            return {
              success: true,
          ...
          case "today": {
            ...
            return {
              success: true,
              message: birthdays.length > 0 ? `Found ${birthdays.length.toString()} birthday(s) today` : "No birthdays today",
              data: { birthdays: data },
            };
          }
          ...
          case "upcoming": {
            ...
            return {
              success: true,
              message: birthdays.length > 0 ? `Found ${birthdays.length.toString()} upcoming birthday(s)` : `No birthdays in the next ${days.toString()} days`,
              data: { birthdays },
            };
          }
          ...
          case "by-month": {
            ...
            return {
              success: true,
              message: birthdays.length > 0 ? `Found ${birthdays.length.toString()} birthday(s) in ${monthName ?? "this month"}` : `No birthdays in ${monthName ?? "this month"}`,
              data: { birthdays: data },
            };
          }
        }
