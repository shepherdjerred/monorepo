  ...
  <div>
              ...
              <div class="card controls">
                <footer class="card-footer">
                  <a class="card-footer-item">
                    <router-link
                      :to="{ name: 'Edit Type', params: { uuid: type.uuid } }"
                    >
                      Edit
                    </router-link>
                  </a>
                  <a class="card-footer-item">
                    <router-link
                      :to="{ name: 'Delete Type', params: { uuid: type.uuid } }"
                    >
                      Delete
                    </router-link>
                  </a>
    ...
    <template v-else>
      <div class="hero is-danger">
        <div class="hero-body">
          <div class="container">
            <h1 class="title">Type not found</h1>
          </div>
        </div>
      </div>
    </template>
...
export default {
  ...
  computed: {
    activitiesWithType: function () {
      return Helpers.objectToArray(this.activities).filter(
        (activity) => activity.type && activity.type.uuid === this.uuid,
      );
    },
    activities: function () {
