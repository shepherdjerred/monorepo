...
/// Detail view showing Anthropic API cost and token usage breakdown.
struct AnthropicDetailView: View {
    // MARK: Internal
    ...
    private var modelTable: some View {
        if self.usage.modelBreakdown.isEmpty {
            ...
            Table(self.usage.modelBreakdown) {
                ...
                .width(120)
                TableColumn("Cache Created") { model in
                    Text(self.formatNumber(model.cacheCreationTokens))
                        .monospacedDigit()
                        .foregroundStyle(.secondary)
                }
                .width(120)
