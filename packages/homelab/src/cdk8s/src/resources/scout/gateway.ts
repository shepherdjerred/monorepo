...
export function assertStageCanHostSplitRoles(
  ...
  throw new Error(
    `Scout ${stage} cannot host split runtime roles: its pinned image ${imageVersion} still stores the database as SQLite at /data/db.sqlite on the shared ReadWriteOnce claim, so a second pod would hold the same SQLite database file open. Promote ${stage} to a PostgreSQL-contract image before adding it to SPLIT_TOPOLOGY_STAGES.`,
  );
}
