...
- mcp-gateway client auth via `mcpProxy.options.authTokens` + a render-config init container (`packages/homelab/src/cdk8s/src/resources/mcp-gateway/`). Deploy needs two manual steps: add 1P field `MCP_PROXY_AUTH_TOKEN` to item `iixelnobjabehkgxhl3ekacdy4`, and add the `Authorization` header to the live (out-of-repo) Claude/Cursor mcp-gateway client config.
