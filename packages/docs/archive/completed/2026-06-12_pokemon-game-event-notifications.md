...
- **Party decryption validated against a real save (2026-06-13).** Loaded a real mid-game Emerald save (`Bl1ndBeholder/pokemon-saves`, 128 KiB after dropping 16 trailing RTC bytes) into the probe. All five party mons decrypted correctly to real species at sensible levels with cleanly-decoded nicknames: SWELLOW "BIRB" L24, COMBUSKEN "KFC" L24, VIGOROTH "SID" L27, SHROOMISH "1-UP" L25, KADABRA "GURU" L24 — five distinct personalities exercising five permutation rows. Badges read as 3 (consistent). This retires the last deferred Phase-0 item (substruct XOR + permutation + checksum, species table, charmap incl. digits/hyphen, badge flags).
- **Pokédex `owned` nuance:** `owned` lives only in SaveBlock2, which the game populates on "Continue", not at the title screen (where SaveBlock1 + party are loaded for the menu preview). At the title screen `gSaveBlock2Ptr` points at a zeroed block, so `dexOwned` reads all-zero — harmless (no events; the title→in-game populate burst is absorbed by the >10-event flood guard). The `owned` offset matches the headers and the national-dex bit-indexing (`national# − 1`) was confirmed against the populated `seen1` copy in SaveBlock1 (all five party members' bits set at the computed indices). In production the bot runs in-game, so SaveBlock2 is populated.
