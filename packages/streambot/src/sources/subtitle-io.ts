 ...
 * pinned to exactly `langPrefs`/`includeAuto`. Shared by the ranking path (`langPrefs` = the
 * server's configured languages) and the exact-pick path (`langPrefs` = a single trackRef
 * language) — a single language list degenerates `pickWrittenSubtitleFile`'s ranking to "pick that
 * one," so this one download routine serves both callers unchanged.
 */
async function downloadYtdlpSubtitle(
  config: Config,
  target: string,
  pick: { langPrefs: readonly string[]; includeAuto: boolean },
  signal: AbortSignal,
): Promise<ResolvedSubtitle | undefined> {
  const { langPrefs, includeAuto } = pick;
  let dir: string;
  try {
...
async function resolveExactTrackForYtdlp(
  ...
  return downloadYtdlpSubtitle(
    config,
    target,
    { langPrefs: [trackRef.lang], includeAuto: trackRef.autoGenerated },
    signal,
  );
}
...
export async function resolveSubtitleForYtdlp(
  ...
  return downloadYtdlpSubtitle(
    ...
    target,
    {
      ...
      includeAuto: config.subtitles.includeAutoGenerated,
    },
    signal,
  );
}
