...
public final class NavigationState {
    ...
    /// Points this window at a destination — what a `tasknotes://` link that
    /// names a place resolves to.
    ///
    /// ⚠️ A link to a **saved view that has been deleted** is accepted here and
    ...
    ///
    /// Parsing deliberately stays in ``RootView``: a link can also name an
    /// *action* — a panel, a window, a focus move — and a router that returned
    /// "handled" for those would be claiming to have moved a selection it never
    /// touched. See ``TaskNotesLink``.
    public func open(_ destination: TaskNotesDestination) {
