...
- **Cold Dagger cache makes CI look hung when it's just slow.** After the force-restart wiped the engine's build cache, the next `main` build (#5376) crawled: each `bun install` in the pokemon smoke test ran 3-8+ min (one crossed 8 min) parked in uninterruptible ZFS `D` state, log static — looked wedged, but every install completed and the build went green. Nearly triggered a second (harmful) engine restart. The pool is healthy and ~19% full; this is ZFS txg write-throughput contention on the nvme build-cache pool under a small-file write storm, not corruption/capacity. Full operational guidance (how to tell slow-vs-wedged, why not to restart the engine) is now in the `buildkite-helper` skill's "This Monorepo's CI Patterns" → "Cold Dagger cache" bullet (`packages/dotfiles/dot_agents/skills/buildkite-helper/SKILL.md`).
