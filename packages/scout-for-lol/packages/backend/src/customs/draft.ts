...
function assignCaptainTeams(
  participant: CustomGameParticipant,
  captainADiscordId: string,
  captainBDiscordId: string,
  clearUnselected: boolean,
): CustomGameParticipant {
  if (participant.discordId === captainADiscordId) {
    return {
      ...participant,
      captain: true,
      team: "A",
  ...
  if (participant.discordId === captainBDiscordId) {
    return {
      ...participant,
      captain: true,
      team: "B",
...
export function selectHostCaptains(
  participants: readonly CustomGameParticipant[],
  captainADiscordId: string,
  captainBDiscordId: string,
): CustomGameParticipant[] {
  requireTen(participants);
  if (captainADiscordId === captainBDiscordId) {
    throw new Error("Custom teams require two distinct captains");
  }
  if (
    !participants.some(
      (participant) => participant.discordId === captainADiscordId,
    ) ||
    !participants.some(
      (participant) => participant.discordId === captainBDiscordId,
    )
  ) {
    throw new Error("Both captains must belong to the current roster");
  }
  return participants.map((participant) =>
    assignCaptainTeams(participant, captainADiscordId, captainBDiscordId, true),
  );
}

export function randomizeCustomTeams(
  participants: readonly CustomGameParticipant[],
  random?: SecureRandom,
): CustomGameParticipant[] {
  requireTen(participants);
  ...
  if (captainA === undefined || captainB === undefined) {
    throw new Error("Select both captains before randomizing teams");
  }
  const others = secureShuffle(
    participants.filter((participant) => !participant.captain),
    random,
  );
  const teamAIds = new Set([
    captainA.discordId,
    ...others.slice(0, 4).map((participant) => participant.discordId),
  ]);
  return participants.map((participant) => ({
    ...participant,
    team: teamAIds.has(participant.discordId) ? "A" : "B",
    side: teamAIds.has(participant.discordId) ? "BLUE" : "RED",
    pickOrder: null,
  }));
}
