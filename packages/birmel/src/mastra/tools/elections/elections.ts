      ...
      try {
        switch (ctx.action) {
          ...
          case "get-stats": {
            ...
            const totalRunoffs = elections.filter(
              (e) => e.pollType === "runoff",
            ).length;
          ...
          case "get-candidate-stats": {
            ...
            for (const e of elections) {
              const candidates = (JSON.parse(e.candidates) as string[]).map(
                (c) => c.toLowerCase(),
              );
            ...
            return {
              ...
              data: {
                ...
                averageVotesPerElection:
                  totalElectionsParticipated > 0
                    ? Math.round(
                        totalVotesReceived / totalElectionsParticipated,
                      )
                    : 0,
                lastElectionDate,
                lastWinDate,
  ...
  outputSchema: z.object({
    ...
    data: z
      .object({
        totalElections: z
          .number()
          .describe("Total number of completed elections"),
        ...
        totalVotesCast: z
          .number()
          .describe("Total votes cast across all elections"),
        ...
        mostRecentWinner: z
          .string()
          .optional()
          .describe("The most recent election winner"),
      })
      .optional(),
  }),
  execute: async (input) => {
