...
<!-- temporal-agent-task
{
  ...
  "checks": [
    { "id": "cutover-timestamp", "label": "Production cutover timestamp", "required": true, "evidenceRequirement": "A dated Comment Log heading explicitly recording the production cutover.", "evidenceCollectors": [{ "id": "cutover-marker", "kind": "command", "argv": ["rg", "--json", "^### 2026-[0-9]{2}-[0-9]{2}.*production cutover", "packages/docs/todos/pagerduty-migration.md"], "output": "non-empty" }] },
    { "id": "retention-window", "label": "Thirty-day retention window", "required": true, "evidenceRequirement": "The recorded cutover marker plus the current UTC timestamp for deterministic elapsed-time calculation.", "evidenceCollectors": [{ "id": "cutover-marker", "kind": "command", "argv": ["rg", "--json", "^### 2026-[0-9]{2}-[0-9]{2}.*production cutover", "packages/docs/todos/pagerduty-migration.md"], "output": "non-empty" }, { "id": "current-time", "kind": "command", "argv": ["date", "-u", "+%Y-%m-%dT%H:%M:%SZ"], "output": "non-empty" }] },
    { "id": "credential-consumers", "label": "Remaining PagerDuty consumers", "required": true, "evidenceRequirement": "Current active-source and live workload credential-reference evidence.", "evidenceCollectors": [{ "id": "active-source-scan", "kind": "command", "argv": ["rg", "--json", "-i", "pagerduty", "packages/homelab", "packages/temporal", "packages/trmnl-dashboard", ".buildkite"], "output": "allow-empty", "successExitCodes": [0, 1] }, { "id": "live-workloads", "kind": "command", "argv": ["kubectl", "get", "deployments,statefulsets,daemonsets,cronjobs", "-A", "-o", "json"], "output": "json" }] }
  ],
  "source": {
