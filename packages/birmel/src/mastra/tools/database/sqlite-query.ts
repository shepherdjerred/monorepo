...
export const manageDatabaseTool = createTool({
  id: "manage-database",
  description: "Manage database: execute SELECT queries or get schema information",
  inputSchema: z.object({
    action: z.enum(["query", "schema"]).describe("The action to perform"),
    query: z.string().optional().describe("SQL SELECT query (for query action)"),
    params: z.array(z.union([z.string(), z.number(), z.boolean(), z.null()])).optional().describe("Query parameters (for query action)"),
    limit: z.number().min(1).max(1000).optional().describe("Max rows to return (for query, default: 100)"),
    tableName: z.string().optional().describe("Specific table name (for schema action)"),
  }),
      ...
      try {
        switch (input.action) {
          case "query": {
            ...
            const rows = await prisma.$queryRawUnsafe<any[]>(finalQuery, ...(input.params ?? []));
            logger.info("SQLite query executed", { rowCount: rows.length });
            return { success: true, message: `Returned ${rows.length.toString()} rows`, data: { rows, count: rows.length } };
          }
          ...
          case "schema": {
            ...
            query += " ORDER BY name";
            const tables = await prisma.$queryRawUnsafe<{ name: string; sql: string }[]>(query, ...params);
            logger.info("Database schema fetched", { tableCount: tables.length });
            return { success: true, message: `Found ${tables.length.toString()} table(s)`, data: { tables } };
          }
        }
      } catch (error) {
