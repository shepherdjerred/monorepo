...
function entry(name: string, value: string, artifactType = "image") {
  return {
    name,
    value,
    category: name.startsWith("shepherdjerred/")
      ? "internal-image"
      : "upstream",
    artifactType,
...
const SAMPLE = JSON.stringify({
  ...
  schemaVersion: 1,
  entries: [
    entry("argo-cd", "9.5.14", "helm-chart"),
    entry(
...
describe("parseCatalogPins", () => {
  test("extracts only first-party 2.0.0-<build> pins", () => {
    const pins = parseCatalogPins(SAMPLE);
  ...
  test("rejects text that is not a valid catalog", () => {
    expect(() => parseCatalogPins("{}")).toThrow();
  });
});
