...
import {
  for_each = {
    for name, project in var.openai_projects : name => project
    if try(project.project_id, null) != null
  }
  to = openai_project.managed[each.key]
...
output "openai_service_account_handoffs" {
  description = "Generated service-account keys and their existing 1Password rotation units"
  sensitive   = true
  value = {
    for name, account in openai_project_service_account.service_accounts : name => {
      api_key             = account.api_key
      onepassword_targets = var.openai_service_accounts[name].onepassword_targets
    }
  }
}
