...
Superseded by the single-workspace architecture. Root `scripts/` is an
intentional Bun workspace and documented repository-automation surface; the
proposed `scripts/ci` package has been removed.

## Comment Log

- 2026-07-27 — Board audit verified root `package.json` and `AGENTS.md` define
  `scripts/` as current architecture. Closed rather than moving the supported
  automation surface.
