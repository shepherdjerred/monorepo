...
export function babysitIterationPrompt(input: BabysitPromptInput): string {
  ...
  return [
    ...
    "  3. No unresolved P3-or-higher review comments (incl. the code-review",
    "     bot (Codex/Greptile), and its 'comments outside of diff').",
    "",
    ...goalLines,
    ...guidanceLines,
    ...
    "- Commit each fix as soon as it works — do NOT batch all commits for the",
    "  end. If you run low on turns, already-committed work is preserved and",
    "  pushed; uncommitted edits are lost.",
    ...
    "- The workdir is a plain clone with NO git hooks installed (the repo has",
    "  none). Validate your fix by running the touched package's",
