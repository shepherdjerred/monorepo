  ...
  return [
    {
      ...
      commands: [
        ...
        "if bun --no-install .buildkite/scripts/selectors/ci-changed.ts site-scout; then scout_source_changed=true; fi",
        'if [ -z "$scout_candidate" ] && [ "$scout_source_changed" != "true" ]; then exit 0; fi',
        ". .buildkite/scripts/toolchain.sh",
        `.buildkite/scripts/bun-install.sh --frozen-lockfile ${SCOUT_WORKSPACE_FILTERS}`,
    ...
    },
    {
      ...
      image: images.base,
      commands: [
        "if ! bun --no-install .buildkite/scripts/selectors/ci-changed.ts scout-reconcile; then exit 0; fi",
        ". .buildkite/scripts/toolchain.sh",
        ".buildkite/scripts/bun-install.sh --frozen-lockfile --filter '@shepherdjerred/root-scripts' --production",
        ...AWS_ALIASES,
