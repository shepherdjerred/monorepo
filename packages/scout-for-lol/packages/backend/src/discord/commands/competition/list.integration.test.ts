...
describe("Competition List Query", () => {
  ...
  test("filters by ownerId", async () => {
    ...
    const owner1Comps = await getCompetitionsByServer(prisma, serverId, {
      ownerId: ownerId1,
    });
    expect(owner1Comps).toHaveLength(1);
    expect(owner1Comps[0]?.title).toBe("Owner 1 Comp");

    const owner2Comps = await getCompetitionsByServer(prisma, serverId, {
      ownerId: ownerId2,
    });
...
describe("Competition List Query - Combined Filters", () => {
