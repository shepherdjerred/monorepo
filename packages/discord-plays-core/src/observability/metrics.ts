...
//
// This is the shared registry + the instruments both discord-plays backends
// define identically. Each game registers its own extra instruments against the
// exported `registry` in its own observability/metrics.ts (e.g. game events for
// pokemon, the richer ffmpeg/send-path metrics for mario-kart), and defines any
// instrument whose help text differs between the games there too (e.g. copyMs).
import {
  Registry,
  Histogram,
  Counter,
...
export const FRAME_MS_BUCKETS = [
  0.25, 0.5, 1, 2, 4, 8, 16, 25, 33, 50, 100, 200,
];
