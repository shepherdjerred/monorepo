...
- Root-caused the `ci/merge-conflict` outage via live HMAC verification
  against a real GitHub delivery (proved the literal placeholder
  `"********"` was the active signing secret).
- `packages/homelab/src/tofu/github/webhooks.tf`: froze both
  `github_repository_webhook.pr_bot` and `.buildkite` with
  `lifecycle { ignore_changes = all }`; verified live via `tofu plan`/`apply`
  (both reported zero diff against real infrastructure).
- Rotated the real `pr_bot` secret: new value set on the live GitHub webhook
  via `gh api`, matching value written to the existing
  `GITHUB_WEBHOOK_SECRET` field on 1Password item `mjgnqqh37jxyzseqrddde2jgaq`
  (no new item, no cdk8s changes). Confirmed the 1Password operator
  auto-restarted the `temporal-worker` pod and a redelivered GitHub payload
  now returns `200` instead of `401`.
- Minted and then revoked two 1Password Connect tokens (`claude-pr-bot-webhook-fix-temp`,
  `Buildkite CI - github tofu stack`) while exploring the abandoned
  tofu-owned-secret approach; neither is in use.

### Remaining

- Optional: build the `PrMergeConflictCheckStale` alert described above (not
  required for the fix; genuine nice-to-have defense-in-depth).
- Backfill `ci/merge-conflict` on currently-open PRs that don't get a
  natural push soon — user decision: leave to organic recovery, no action
  needed unless that changes.
- Open the PR for the `webhooks.tf` fix (in progress).

### Caveats

- `kubectl port-forward` to the Temporal frontend is unreliable from this
  session's sandbox (namespace-loopback dial refused) — the manual
  `all-prs` backfill needs to be run from a normal operator terminal if
  ever wanted sooner than organic recovery.
- The two revoked Connect tokens (`claude-pr-bot-webhook-fix-temp`,
  `Buildkite CI - github tofu stack`) still exist in the Kubernetes Connect
  server's token list in `REVOKED` state — harmless, but worth knowing
  they're there if someone audits `op connect token list --server Kubernetes`.
