...
- [ ] **Verify alert delivery**: `StreambotEncoderFallingBehind` (speed < 0.95,
...
- [ ] **Pod-lifecycle forensics gap**: the event exporter drops all Normal-type
...
- [ ] **Node-level DRM-clients exporter** for whole-GPU tenancy visibility
      (per-pod fdinfo is blind to other tenants; `intel_gpu_top` is broken on
      Raptor Lake — see intel/media-driver#1376).
- [ ] **Residual stutter check with the new gauge**: after PR #1542 deploys,
