      ...
      try {
        ...
        switch (ctx.action) {
          case "send": {
            ...
            const styledContent = stylizeContent(
              ctx.content,
              requestContext?.guildId,
            );
          ...
          case "reply": {
            ...
            ).messages.fetch(requestContext.sourceMessageId);
            const styledContent = stylizeContent(
              ctx.content,
              requestContext.guildId,
            );
          ...
          case "edit": {
            if (!ctx.channelId || !ctx.messageId || !ctx.content) {
              return {
                success: false,
                message:
                  "channelId, messageId, and content are required for edit",
              };
            }
            const channel = await client.channels.fetch(ctx.channelId);
            if (!channel?.isTextBased()) {
          ...
          case "bulk-delete": {
            if (!ctx.channelId || !ctx.messageIds?.length) {
              return {
                success: false,
                message:
                  "channelId and messageIds are required for bulk-delete",
              };
            }
            ...
            if (ctx.messageIds.length > 100) {
              return {
                success: false,
                message:
                  "Cannot delete more than 100 messages at once (Discord limit)",
              };
            }
            const channel = await client.channels.fetch(ctx.channelId);
            if (!channel?.isTextBased()) {
            ...
            logger.info("Messages bulk deleted", {
              channelId: ctx.channelId,
              count: ctx.messageIds.length,
            });
            return {
              success: true,
