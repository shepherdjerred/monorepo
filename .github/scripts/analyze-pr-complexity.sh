...
# - Simple: <200 lines, <5 files → 10 turns
# - Medium: <500 lines, <10 files → 15 turns
# - Complex: >=500 lines OR >=10 files → 30 turns

if [ "$TOTAL_CHANGES" -lt 200 ] && [ "$CHANGED_FILES" -lt 5 ]; then
  MAX_TURNS=10
  COMPLEXITY="simple"
  echo "  Classification: SIMPLE (<200 lines, <5 files)"
elif [ "$TOTAL_CHANGES" -lt 500 ] && [ "$CHANGED_FILES" -lt 10 ]; then
  MAX_TURNS=15
  COMPLEXITY="medium"
  echo "  Classification: MEDIUM (<500 lines, <10 files)"
else
  MAX_TURNS=30
  COMPLEXITY="complex"
  echo "  Classification: COMPLEX (≥500 lines OR ≥10 files)"
fi
