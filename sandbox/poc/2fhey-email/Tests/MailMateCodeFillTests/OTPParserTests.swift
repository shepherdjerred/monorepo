...
func rejectsFalsePositives() {
    ...
    let localizedDateMetadata = MessageMetadata(sender: "Seguridad <security@example.test>", subject: "Código de verificación", date: nil, messageID: "message-localized-date")
    #expect(OTPParser().parse(body: "Caduca el 20 de agosto de 2026.", metadata: localizedDateMetadata) == nil)
    #expect(OTPParser().parse(body: "Expire le 20 août 2026.", metadata: localizedDateMetadata) == nil)
    #expect(OTPParser().parse(body: "Läuft am 20. August 2026 ab.", metadata: localizedDateMetadata) == nil)
    ...
    #expect(OTPParser().parse(body: "Copyright 2026 Acme\n482913", metadata: subjectLabelMetadata)?.code == "482913")
