...
export const setVolumeTool = createTool({
  ...
  execute: async ({ context }) => {
    ...
    try {
      ...
      const queue = player.queues.get(context.guildId);
      ...
      queue.node.setVolume(context.volume);

      return {
        success: true,
        message: `Volume set to ${String(context.volume)}%`,
      };
    } catch (error) {
...
export const setLoopModeTool = createTool({
  ...
  execute: async ({ context }) => {
    ...
    try {
      ...
      queue.setRepeatMode(modeMap[context.mode]);

      return {
        success: true,
        message: `Loop mode set to ${context.mode}`,
      };
    } catch (error) {
...
export const seekTool = createTool({
  ...
  execute: async ({ context }) => {
    try {
      ...
      const success = await queue.node.seek(context.seconds * 1000);
      ...
      const minutes = Math.floor(context.seconds / 60);
      const secs = context.seconds % 60;
