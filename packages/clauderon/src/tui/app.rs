...
pub enum CreateDialogFocus {
    ...
    /// Plan mode checkbox.
    PlanMode,
    /// Dangerous copy credentials checkbox.
    DangerousCopyCreds,
    /// Container image field (K8s).
    ContainerImage,
    /// Image pull policy field (K8s).
    PullPolicy,
    /// Storage class field (K8s).
    StorageClass,
...
#[derive(Debug, Clone)]
#[expect(clippy::struct_excessive_bools, reason = "dialog state naturally has many boolean fields")]
pub struct CreateDialogState {
    ...
    /// Cursor line position in prompt field.
    pub prompt_cursor_line: usize,
    /// Cursor column position in prompt field.
    pub prompt_cursor_col: usize,
    /// Scroll offset for prompt field.
    pub prompt_scroll_offset: usize,
    /// Current input focus in the dialog.
    pub focus: CreateDialogFocus,
    /// Whether Create button is focused (true) or Cancel (false).
...
impl CreateDialogState {
    ...
    /// Reset dialog state to defaults, preserving feature flags.
...
#[expect(clippy::struct_excessive_bools, reason = "TUI app state naturally has many boolean flags")]
pub struct App {
    /// Current mode/view
    pub mode: AppMode,
...
impl App {
    ...
    pub async fn switch_to_next_session(&mut self) -> anyhow::Result<bool> {
        ...
        let container_id = next_session
            .backend_id
            .clone()
            .ok_or_else(|| anyhow::anyhow!("session has no backend_id"))?;
