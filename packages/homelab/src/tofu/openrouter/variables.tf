variable "openrouter_workspaces" {
  ...
  type = map(object({
    ...
    io_logging_api_key_ids              = optional(list(number))
...
variable "openrouter_api_keys" {
  ...
  type = map(object({
    ...
    onepassword_targets = optional(list(object({
  ...
  validation {
    condition = alltrue([
      for api_key in values(var.openrouter_api_keys) :
      try(api_key.import_id, null) != null || length(api_key.onepassword_targets) > 0
    ])
    error_message = "Every generated OpenRouter API key must declare at least one 1Password target."
  }
}
...
variable "openrouter_byok_credentials" {
  ...
  type = map(object({
    ...
    allowed_api_key_hashes = optional(list(string))
  }))
  default = {}
}
