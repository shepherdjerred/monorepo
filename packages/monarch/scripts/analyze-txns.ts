...
for (const t of txns) {
  ...
  g.categories.set(
    t.category.name,
    (g.categories.get(t.category.name) ?? 0) + 1,
  );
...
console.log(
  "=== MULTI-CATEGORY MERCHANTS (same merchant, different categories) ===\n",
);
for (const [name, g] of sorted) {
  if (g.categories.size > 1) {
    const cats = [...g.categories.entries()]
      ...
      .join(", ");
    console.log(
      `  ${name.padEnd(35)} ${String(g.count).padStart(4)} txns  $${g.total.toFixed(2).padStart(10)}  → ${cats}`,
    );
  }
}
...
const byCategory = new Map<
  string,
  { merchants: string[]; count: number; total: number }
>();
for (const [name, g] of sorted) {
  for (const [cat, count] of g.categories) {
    if (
      genericCategories.some((gc) =>
...
for (const [cat, entry] of byCategory) {
  console.log(
    `  ${cat}: ${String(entry.count)} txns, $${entry.total.toFixed(2)}`,
  );
  for (const m of entry.merchants.slice(0, 15)) {
    console.log(`    - ${m}`);
  }
  if (entry.merchants.length > 15)
    console.log(`    ... and ${String(entry.merchants.length - 15)} more`);
}
...
const ambiguous = sorted.filter(([name]) =>
  ambiguousPatterns.some((p) => name.toLowerCase().includes(p)),
);
