...
describe("commandScopes", () => {
  test("keeps a subshell export out of scripts that run outside it", () => {
    ...
    const scopes = commandScopes(
      [
        ...
        "  bun packages/homelab/scripts/tofu/tofu-stack.ts seaweedfs plan",
        ")",
        "bun scripts/release/deploy-site.ts wiki --dry-run",
      ].join("\n"),
    );
    const inside = scopes.find((scope) =>
      scope.scripts.includes("packages/homelab/scripts/tofu/tofu-stack.ts"),
    );
    const outside = scopes.find((scope) =>
      scope.scripts.includes("scripts/release/deploy-site.ts"),
    );
  ...
  test("a command substitution is not treated as a subshell", () => {
    ...
    const scopes = commandScopes(
      [
        'export DIGEST="$$(bun scripts/release/release.ts resolve)"',
        "bun scripts/release/deploy-site.ts wiki",
      ].join("\n"),
    );
    expect(scopes).toHaveLength(1);
    expect(scopes[0]?.assigned.has("DIGEST")).toBe(true);
    expect(scopes[0]?.scripts).toContain("scripts/release/deploy-site.ts");
  });

  test("an export before a subshell is still visible inside it", () => {
    const scopes = commandScopes(
      ['export SHARED="x"', "(", "  bun scripts/release/release.ts", ")"].join(
        "\n",
      ),
    );
    const inner = scopes.find((scope) =>
      scope.scripts.includes("scripts/release/release.ts"),
    );
...
describe("scriptPathsInCommand", () => {
  ...
  /**
   * A one-level-only pattern drops a nested entry point silently: the
   * credential gate keeps passing while analyzing fewer scripts. Nothing else
   * fails, so only an explicit test catches it.
   */
  test("finds an entry point nested inside its scripts root", () => {
    expect(
  ...
  test("still finds an entry point directly in its scripts root", () => {
    expect(scriptPathsInCommand("bun scripts/verify.ts")).toEqual([
      "scripts/verify.ts",
    ]);
  });
});
              ...
              - name: container-0
                env:
                  - name: BUILDKITE_SHELL
                    value: /bin/bash -e -c
  - key: no-secret
    command: bun scripts/release/publish-npm.ts
    plugins:
      - kubernetes:
          podSpecPatch:
            containers:
...
`,
  ...
  test("records container env and global env per step", () => {
    ...
    expect(withEnv?.scripts).toEqual(["scripts/release/release.ts"]);
  });
});
...
describe("collectErrors", () => {
  const step = {
    ...
    scripts: ["scripts/release/release.ts"],
  };
  ...
  test("passes a name the step's command assigns", () => {
    const errors = collectErrors({
      steps: [step],
      requiredFor: () =>
        required({ EXPORTED_NAME: "scripts/release/release.ts:10" }),
    });
  ...
  test("reports a requireEnv whose argument is not a literal", () => {
    const errors = collectErrors({
      steps: [step],
      requiredFor: () => required({}, ["scripts/release/release.ts:220"]),
    });
    expect(errors).toHaveLength(1);
  ...
  test("does not apply one script's exception names to another script", () => {
    ...
    const deployErrors = collectErrors({
      steps: [{ ...step, scripts: ["scripts/release/deploy-site.ts"] }],
      requiredFor: () => required({}),
      dynamicCallExceptions: exceptions,
    });
  ...
  test("accepts a declared dynamic-call exception and still checks its names", () => {
    // scripts/release/deploy-site.ts is the declared exception; its unresolved call is
    // silent, but any name the exception declares is checked like any other.
    const deployStep = { ...step, scripts: ["scripts/release/deploy-site.ts"] };
    const errors = collectErrors({
      steps: [deployStep],
      requiredFor: () => required({}, ["scripts/release/deploy-site.ts:220"]),
    });
    expect(errors).toEqual([]);
  });
});
