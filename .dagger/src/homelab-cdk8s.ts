...
import { caddyVersionOnly } from "./lib-versions.ts";
 ...
 */
export function typeCheckCdk8sWithContainer(
  container: Container,
): Promise<string> {
  return execOrThrow(container, ["bun", "run", "typecheck"]);
}
...
export function buildK8sManifests(source: Directory): Directory {
  const builtContainer = prepareCdk8sContainer(source).withExec([
    "bun",
    "run",
    "build",
  ]);
 ...
 */
export function buildK8sManifestsWithContainer(
  container: Container,
): Directory {
...
export async function validateCaddyfile(source: Directory): Promise<string> {
  ...
  const caddyfileContent = await cdk8sContainer
    .withExec(["bun", "run", "scripts/generate-caddyfile.ts"])
    .stdout();
  ...
  return execOrThrow(caddyContainer, [
    "caddy",
    "validate",
    "--config",
    "/etc/caddy/Caddyfile",
 ...
 */
export async function validateCaddyfileWithContainer(
  container: Container,
): Promise<string> {
  // Generate the Caddyfile
  const caddyfileContent = await container
    .withExec(["bun", "run", "scripts/generate-caddyfile.ts"])
    .stdout();

  // Validate using caddy-s3proxy container
  const caddyContainer = buildCaddyS3ProxyContainer().withNewFile(
    "/etc/caddy/Caddyfile",
    caddyfileContent,
  );
