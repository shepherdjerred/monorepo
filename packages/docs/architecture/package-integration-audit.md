...
- **obsidian-sync-client** — Obsidian vault sync CLI/server
- **tasknotes-server** — TaskNotes HTTP API server

## Integration Matrix

| Area | sentinel | tasks-for-obsidian | obsidian-sync-client | tasknotes-server |
|---|---|---|---|---|
| Workspace member | Yes | No (by design) | No (by design) | No (by design) |
| CLAUDE.md | Yes | Yes | Yes | Yes |
| Package scripts (build/test/lint/typecheck) | Yes | Yes | Yes | Yes |
| ESLint shared config | Yes | Yes (reactNative) | Yes | Yes |
| tsconfig extends base | Yes | Yes | Yes | No (standalone) |
| Dagger CI | Yes (tier 0) | **Missing** | Yes (tier 0) | Yes (tier 0) |
| Docker/GHCR publish | Yes | N/A (RN app) | Yes | Yes |
| Lefthook pre-commit | **Missing** | **Missing** | **Missing** | **Missing** |
| Knip unused-code | Unclear | Ignored | **Missing** | **Missing** |
...
3. **obsidian-sync-client and tasknotes-server missing from knip** — no unused-code analysis
...
- sentinel, obsidian-sync-client, and tasknotes-server have excellent Dagger CI with parallel checks, Docker builds, smoke tests, and GHCR publishing
