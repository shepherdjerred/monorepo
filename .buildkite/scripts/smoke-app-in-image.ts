...
> = {
  ...
  "openrouter-broadcast-ingest": {
    command: [
      "set -eu",
      "cd /app/packages/openrouter-broadcast-ingest",
      "bun src/index.ts >/tmp/openrouter-broadcast-smoke.log 2>&1 &",
      "pid=$!",
      "trap 'if kill -0 $pid; then kill $pid; if wait $pid; then :; else cleanup_status=$?; [ $cleanup_status -eq 143 ] || exit $cleanup_status; fi; fi' EXIT",
      "for _ in $(seq 1 30); do",
      `  if bun -e 'const app = await fetch("http://127.0.0.1:18792/readyz"); const metrics = await fetch("http://127.0.0.1:18793/metrics"); if (!app.ok || !metrics.ok || !(await metrics.text()).includes("openrouter_broadcast_requests_total")) process.exit(1);'; then exit 0; fi`,
      "  if ! kill -0 $pid; then cat /tmp/openrouter-broadcast-smoke.log; exit 1; fi",
      "  sleep 1",
      "done",
      "cat /tmp/openrouter-broadcast-smoke.log",
      "exit 1",
    ].join("\n"),
    env: {
      ...
      METRICS_PORT: "18793",
      OPENROUTER_BROADCAST_BEARER_TOKEN:
        "smoke-broadcast-token-that-is-long-enough",
      PORT: "18792",
      S3_ENDPOINT: "http://127.0.0.1:19000",
      TEMPO_OTLP_HTTP_URL: "http://127.0.0.1:19001/v1/traces",
    },
  },
  birmel: {
    command: [
  ...
  "temporal-worker": {
    command: [
      ...
      "gh --version",
      `bun -e 'await import("@anthropic-ai/claude-agent-sdk"); await import("@openai/codex-sdk");'`,
