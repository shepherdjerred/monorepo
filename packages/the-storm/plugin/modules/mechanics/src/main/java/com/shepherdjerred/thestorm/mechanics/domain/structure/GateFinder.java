...
/**
 * Finds the gate a {@code [Gate]} sign controls.
 *
 ...
 * move. The top of each column always stays; below it the column fills down through air and water
 * until it meets anything else, at most {@code maxHeight} blocks.
 */
public final class GateFinder {
  ...
  public static Result<Structure, StructureProblem> find(
      BlockGrid grid, Pos sign, GateConfig config) {
    ...
    var nearest = nearestColumn.getKey();
    var material = nearestColumn.getValue();
    var chosen =
        tops.entrySet().stream()
            ...
            .toList();
    var allTops = Set.copyOf(tops.keySet());
    var cells = new ArrayList<Pos>();
    for (var top : chosen) {
      cells.addAll(column(grid, top, material, new ColumnLimits(config.maxHeight(), allTops)));
    }
    return Result.ok(new Structure(material, nearest, cells));
  }
