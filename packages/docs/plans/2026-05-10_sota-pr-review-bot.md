...
- **Metric rename per team-lead → align with eval's dashboard namespace** (drop the `_pipeline_` infix; "pipeline" is implied by `pr_review_`): `pr_review_pipeline_posted_total` → `pr_review_count_total`, `pr_review_pipeline_findings_per_pr` → `pr_review_findings_per_pr`. Variable names updated to match (`prReviewCountTotal`, `prReviewFindingsPerPr`). Eval's Phase 8 Grafana panels target the `pr_review_*` namespace; renaming now (before any series accumulates) avoids back-filling labels later and keeps the histogram aligned with the canonical plan's architecture diagram.
