...
pub(super) async fn handle_copy_mode_key(app: &mut App, key: KeyEvent) -> anyhow::Result<()> {
    ...
    let Some(state) = app.copy_mode_state.as_mut() else {
        return Ok(());
    };
    ...
    match key.code {
        ...
        // Yank (copy) selection
        KeyCode::Char('y') if state.visual_mode => {
...
fn copy_selection_to_clipboard(app: &App) -> anyhow::Result<()> {
    ...
    let Some(state) = app.copy_mode_state.as_ref() else {
        anyhow::bail!("copy mode state not initialized");
    };
