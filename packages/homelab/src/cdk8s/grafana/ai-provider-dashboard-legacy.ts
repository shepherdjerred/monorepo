...
} from "./ai-provider-dashboard-panels.ts";

function activeIssueExpr(filter: string): string {
  return `sum(max by (app, provider, kind, source) (ai_provider_issue_active{${filter}})) or on() vector(0)`;
}

export function addProviderIssuePanels(
  builder: dashboard.DashboardBuilder,
): void {
  const filter =
    'app=~"$app",provider=~"$provider",kind=~"$kind",source=~"$source"';
  const filterWithoutKind =
    'app=~"$app",provider=~"$provider",source=~"$source"';
  ...
  builder.withRow(
    new dashboard.RowBuilder("Overview").gridPos({
      x: 0,
      y: 0,
      w: 24,
      h: 1,
  ...
  builder.withRow(
    new dashboard.RowBuilder("Issues").gridPos({
      x: 0,
      y: 5,
      w: 24,
      h: 1,
  ...
  builder.withRow(
    new dashboard.RowBuilder("Errors").gridPos({
      x: 0,
      y: 14,
      w: 24,
      h: 1,
  ...
  builder.withPanel(
    createTimeseriesPanel({
      ...
      description:
        "Scout token-budget circuit breaker events per hour. This captures expected spend-control skips outside Bugsink.",
      targets: [
        {
  ...
  builder.withPanel(
    createTimeseriesPanel({
      ...
      description:
        "Input/context token limit errors per hour. These indicate oversized prompts or unusually large match timelines.",
      targets: [
        {
